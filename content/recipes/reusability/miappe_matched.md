...
long term in public repositories with sufficient metadata information to make conclusions about them, in line with FAIR (URL_TO_INSERT_RECORD-ABBREV_2453 https://fairsharing.org/FAIRsharing.WWI10U) 
principles. 
...
data, with a particular focus on plants.  This includes:
    ...
    2) Submission of sequencing data and metadata to ENA (URL_TO_INSERT_RECORD-ABBREV_2454 https://fairsharing.org/FAIRsharing.dj8nt8) .
    3) Retrieval of the correct genome assembly for the genotyping experiment
    4) Conversion of the resulting analysis file (in VCF (URL_TO_INSERT_RECORD-ABBREV_2456 https://fairsharing.org/FAIRsharing.cfzz0h)  format) to be FAIR (URL_TO_INSERT_RECORD-ABBREV_2455 https://fairsharing.org/FAIRsharing.WWI10U) 
    5) Submission of the genotyping results to EVA (URL_TO_INSERT_RECORD-ABBREV_2457 https://fairsharing.org/FAIRsharing.6824pv) .

In terms of FAIRification goals, this means obtaining stable, resolvable  identifiers for the datasets and meeting
community annotation requirements as expressed in the MIAPPE (URL_TO_INSERT_RECORD-ABBREV_2458 https://fairsharing.org/FAIRsharing.nd9ce9)  requirements. 
...
The exact listing of the metadata fields required for a FAIRification of the genotyping data set within a VCF (URL_TO_INSERT_RECORD-ABBREV_2459 https://fairsharing.org/FAIRsharing.cfzz0h)  file is 
...
This is done in general through JSON (URL_TO_INSERT_RECORD-ABBREV_2460 https://fairsharing.org/FAIRsharing.5bbab9)  API ([Python](https://github.com/PBR/elixir-fondue-datathon/tree/master/test_data_set/BioSamples/scripts/python) 
...
to make downstream submissions to ENA (URL_TO_INSERT_RECORD-ABBREV_2461 https://fairsharing.org/FAIRsharing.dj8nt8) , to use the Webin Authentication service. You can create a Webin account using 
...
2. Login to the system and get a JSON (URL_TO_INSERT_RECORD-ABBREV_2462 https://fairsharing.org/FAIRsharing.5bbab9)  Web Token (JWT)

Webin uses JSON (URL_TO_INSERT_RECORD-ABBREV_2463 https://fairsharing.org/FAIRsharing.5bbab9)  Web Token (JWT) for authentication. Use your login credentials to retrieve a JWT. You can either use 
...
Submit Sequencing reads to ENA (URL_TO_INSERT_RECORD-ABBREV_2464 https://fairsharing.org/FAIRsharing.dj8nt8) , using BioSamples IDs to identify material.


#### 4.1 The Study

To begin, you should register a study. Recall that a study describes the purpose of the work you have done, groups other
objects beneath it, and controls when the data becomes public. A study is required for all submissions to ENA (URL_TO_INSERT_RECORD-ABBREV_2465 https://fairsharing.org/FAIRsharing.dj8nt8) .
...
In ENA (URL_TO_INSERT_RECORD-ABBREV_2466 https://fairsharing.org/FAIRsharing.dj8nt8) , samples are required to conform to a checklist of values. Checklists define a set of mandatory and recommended 
...
4. Select the appropriate checklist. For general purposes ERC000037 (**‘ENA (URL_TO_INSERT_RECORD-ABBREV_2467 https://fairsharing.org/FAIRsharing.dj8nt8)  Plant Sample Checklist’**) should be the 
default when submitting plant sequencing data. Now click '**Next**'.
add an image of the web ui for https://www.ebi.ac.uk/ena (URL_TO_INSERT_RECORD-HOMEPAGE_2468 https://fairsharing.org/FAIRsharing.dj8nt8) /browser/view/ERC000037
...
* If yes, proceed directly to VCF (URL_TO_INSERT_RECORD-ABBREV_2469 https://fairsharing.org/FAIRsharing.cfzz0h)  submission at step 4.
* If no, [submit reference genome assembly](https://ena-docs.readthedocs.io/en/latest/submit/assembly/genome.html) 
to [INSDC](https://www.insdc.org/) (NCBI Genbank / EMBL-EBI ENA (URL_TO_INSERT_RECORD-ABBREV_2471 https://fairsharing.org/FAIRsharing.dj8nt8)  / DDBJ (URL_TO_INSERT_RECORD-ABBREV_2470 https://fairsharing.org/FAIRsharing.k337f0) ) and wait until accession number is issued,
...
In order to ensure interoperability of VCF (URL_TO_INSERT_RECORD-ABBREV_2472 https://fairsharing.org/FAIRsharing.cfzz0h)  files, in accordance with the good practice outlined in 
{footcite}`beier_f1000r_2022`, the following VCF (URL_TO_INSERT_RECORD-ABBREV_2473 https://fairsharing.org/FAIRsharing.cfzz0h)  meta-information lines should be used:
...
**##fileDate(Date)**: creation date of the VCF (URL_TO_INSERT_RECORD-ABBREV_2474 https://fairsharing.org/FAIRsharing.cfzz0h)  in the basic form without separator: YYYYMMDD 

> Example:
>  `##fileDate=20120921` 

**##bioinformatics_source (URL (URL_TO_INSERT_RECORD-ABBREV_2476 https://fairsharing.org/FAIRsharing.9d38e2)  or URI (URL_TO_INSERT_RECORD-ABBREV_2475 https://fairsharing.org/FAIRsharing.d261e1) )**: Analytic approach usually consisting of chains of bioinformatics tools for 
creating the VCF (URL_TO_INSERT_RECORD-ABBREV_2480 https://fairsharing.org/FAIRsharing.cfzz0h)  file specified as the DOI (URL_TO_INSERT_RECORD-ABBREV_2477 https://fairsharing.org/FAIRsharing.hFLKCn)  of a publication, or more generally as URL (URL_TO_INSERT_RECORD-ABBREV_2479 https://fairsharing.org/FAIRsharing.9d38e2) /UR (URL_TO_INSERT_RECORD-ABBREV_2478 https://fairsharing.org/FAIRsharing.d261e1) I, like a public repository for
the scripts used. The preferred way to describe this would be to use WorkflowHub (URL_TO_INSERT_RECORD-NAME_2482 https://fairsharing.org/FAIRsharing.07cf72) .eu (possibly in CWL (URL_TO_INSERT_RECORD-ABBREV_2481 https://fairsharing.org/FAIRsharing.8y5ayx)  format) and to be
...
which the variation data of the present VCF (URL_TO_INSERT_RECORD-ABBREV_2483 https://fairsharing.org/FAIRsharing.cfzz0h)  is based. 

> Example: 
> ` ##reference_ac=GCA_902498975.1  `

**##reference_url (DOI (URL_TO_INSERT_RECORD-ABBREV_2484 https://fairsharing.org/FAIRsharing.hFLKCn) )**: a DOI (URL_TO_INSERT_RECORD-ABBREV_2485 https://fairsharing.org/FAIRsharing.hFLKCn)  (or URL (URL_TO_INSERT_RECORD-ABBREV_2487 https://fairsharing.org/FAIRsharing.9d38e2) /UR (URL_TO_INSERT_RECORD-ABBREV_2486 https://fairsharing.org/FAIRsharing.d261e1) I) for downloading of this reference genome, preferably from one INSDC archive. 
...
**##SAMPLE(<ID=$BioSample_accession, DOI (URL_TO_INSERT_RECORD-ABBREV_2488 https://fairsharing.org/FAIRsharing.hFLKCn) =$url, ext_ID=$registry:identifier>)** : Describe the material whose variants 
are given in the genotype call columns in greater detail and can be extended using the specifications of the VCF (URL_TO_INSERT_RECORD-ABBREV_2489 https://fairsharing.org/FAIRsharing.cfzz0h)  format.

In case no DOI (URL_TO_INSERT_RECORD-ABBREV_2490 https://fairsharing.org/FAIRsharing.hFLKCn)  exists and the material is held by a FAO (URL_TO_INSERT_RECORD-ABBREV_2491 https://fairsharing.org/FAIRsharing.xs6t67) -WIEWS ([https://www.fao.org/wiews/background/en/](https://www.fao.org/wiews/background/en/)) 
recognised institution, the external ID consists of the FAO (URL_TO_INSERT_RECORD-ABBREV_2492 https://fairsharing.org/FAIRsharing.xs6t67) -WIEWS instcode, the genus and the accession number (see example 2).
If the database is not registered with FAO (URL_TO_INSERT_RECORD-ABBREV_2494 https://fairsharing.org/FAIRsharing.xs6t67) -WIEWS and is not available under a DOI (URL_TO_INSERT_RECORD-ABBREV_2493 https://fairsharing.org/FAIRsharing.hFLKCn) , the DNS of the holding institution,
...
> Examples:
>
> * One genotype from a barley (_Hordeum vulgare_) GBS experiment with a DOI (URL_TO_INSERT_RECORD-ABBREV_2495 https://fairsharing.org/FAIRsharing.hFLKCn)  registered.
>
>       `##SAMPLE=<ID=SAMEA104646767,DO (URL_TO_INSERT_RECORD-ABBREV_2496 https://fairsharing.org/FAIRsharing.hFLKCn) I="doi.org/10.25642/IPK/GBIS/7811152">`
>
> 
> * One genotype from a barley (_Hordeum vulgare_) GBS experiment with the FAO (URL_TO_INSERT_RECORD-ABBREV_2498 https://fairsharing.org/FAIRsharing.xs6t67) -WIEWS code available but no DOI (URL_TO_INSERT_RECORD-ABBREV_2497 https://fairsharing.org/FAIRsharing.hFLKCn) .
>
>       `##SAMPLE=<ID=SAMEA104646767,ext_ID="DEU146:Hordeum:HOR 1361 BRG">`
> 
> 
> * One genotype from a barley (_Hordeum vulgare_) GBS experiment with no DOI (URL_TO_INSERT_RECORD-ABBREV_2499 https://fairsharing.org/FAIRsharing.hFLKCn)  and no FAO (URL_TO_INSERT_RECORD-ABBREV_2500 https://fairsharing.org/FAIRsharing.xs6t67) -WIEWS code available.
> 
...
Once the metadata and data has been formatted according to the specifications above, make sure that the resulting VCF (URL_TO_INSERT_RECORD-ABBREV_2501 https://fairsharing.org/FAIRsharing.cfzz0h)  
file complies with VCF (URL_TO_INSERT_RECORD-ABBREV_2502 https://fairsharing.org/FAIRsharing.cfzz0h)  specifications. 
...
Once the file has been fully validated without any error messages, you can submit the VCF (URL_TO_INSERT_RECORD-ABBREV_2504 https://fairsharing.org/FAIRsharing.cfzz0h)  file to EVA (URL_TO_INSERT_RECORD-ABBREV_2503 https://fairsharing.org/FAIRsharing.6824pv) , using 
BioSamples IDs to identify the material, GCF/GCA accession for the reference genome assembly, and ENA (URL_TO_INSERT_RECORD-ABBREV_2505 https://fairsharing.org/FAIRsharing.dj8nt8)  accession numbers
for the sequencing reads of the material used. Refer to the official [documentation](https://www.ebi.ac.uk/eva/?Submit-Data).
   
 
## Conclusion
At this point, the VCF (URL_TO_INSERT_RECORD-ABBREV_2506 https://fairsharing.org/FAIRsharing.cfzz0h)  contains metadata and data formatted for the purpose of better discoverability and higher 
interoperability. 
Data could thus be more easily read and evaluated automatically by machines, and it is made easier to 
connect different data sources with each other, so that in general a higher degree of FAIR (URL_TO_INSERT_RECORD-ABBREV_2507 https://fairsharing.org/FAIRsharing.WWI10U)  has been achieved.
