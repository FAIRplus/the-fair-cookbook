...
The purpose of this recipe is to show how to create a metadata collection form complying with a community minimal information checklist (MIUViG (URL_TO_INSERT_RECORD-ABBREV_1676 https://fairsharing.org/FAIRsharing.bd9566) ), in the context of Covid-19 strain sequencing assays carried on patient collected samples.
In addition, the recipe includes the conversion of sample metadata to an RDF (URL_TO_INSERT_RECORD-ABBREV_1677 https://fairsharing.org/FAIRsharing.p77ph9) /Linked Data graph and checks its structure for conformance to requirement using the ShapeExpression specifications.
Finally, use queries expressed in SPARQL (URL_TO_INSERT_RECORD-ABBREV_1678 https://fairsharing.org/FAIRsharing.87ccfd)  are shown to demonstrate potential data integration scenarios.
...
|:--|:--|
| [Semantic Annotation for Linked Arvado Data (SALAD)](https://www.commonwl.org/draft-3/SchemaSalad.html)  | conversion from  YAML to RDF (URL_TO_INSERT_RECORD-ABBREV_1679 https://fairsharing.org/FAIRsharing.p77ph9)  |
|[WESO shExVisualize](http://rdfshape.weso.es/shExVisualize) | Shape expression syntax visualization |
| [Virtuoso](https://virtuoso.openlinksw.com/) | RDF (URL_TO_INSERT_RECORD-ABBREV_1680 https://fairsharing.org/FAIRsharing.p77ph9)  triple store |
...
> * Expressing instance data in RDF (URL_TO_INSERT_RECORD-ABBREV_1682 https://fairsharing.org/FAIRsharing.p77ph9) /linked data format
> * Validating RDF (URL_TO_INSERT_RECORD-ABBREV_1683 https://fairsharing.org/FAIRsharing.p77ph9)  instance data against requirements using a Shape Expression(SHEX)
> * Testing query cases by formulating SPARQL (URL_TO_INSERT_RECORD-ABBREV_1684 https://fairsharing.org/FAIRsharing.87ccfd)  queries
...
<br>Based on the [Genome Standards Consortium](https://gensc.org/) metadata requirement profile for uncultivated viral sample, also known as the [Minimum Information About an Uncultivated Virus Genome (URL_TO_INSERT_RECORD-NAME_1685 https://fairsharing.org/FAIRsharing.bd9566)  (MIUViG (URL_TO_INSERT_RECORD-ABBREV_1686 https://fairsharing.org/FAIRsharing.bd9566) )](https://github.com/GenomicsStandardsConsortium/mixs/blob/master/UViGs_MIxS.xlsx), the first step is to anchor the tags defined by GSC (URL_TO_INSERT_RECORD-ABBREV_1687 https://fairsharing.org/3528)  and approved by the those **International Nucleotide Sequence Database Collaboration** ([INSDC](http://www.insdc.org/)) tags to one (or more) semantic framework(s).
...
However, for the final implementation, only the OBO (URL_TO_INSERT_RECORD-ABBREV_1688 https://fairsharing.org/FAIRsharing.847069)  related mappings have been used as show in the following figure.
...
Quoting the project's documentation, "*[the Semantic Annotations for Linked Avro Data (SALAD)](https://github.com/common-workflow-language/schema_salad) is a schema language for describing JSON (URL_TO_INSERT_RECORD-ABBREV_1689 https://fairsharing.org/FAIRsharing.5bbab9)  or YAML structured linked data documents. `SALAD schema` describes rules for preprocessing, structural validation, and hyperlink checking for documents described by a Salad schema. Salad supports rich data modeling with inheritance, template specialization, object identifiers, object references, documentation generation, code generation, and transformation to RDF (URL_TO_INSERT_RECORD-ABBREV_1690 https://fairsharing.org/FAIRsharing.p77ph9) . SALAD provides a bridge between document and record oriented data modeling and the Semantic Web."*
...
But in this example, we are using the SALAD schema to capture the annotation requirements in a [YAML](https://yaml.org) document, while also embedding the semantics constraints, which can then be used to  to build a web form (see below) but also support conversion to RDF (URL_TO_INSERT_RECORD-ABBREV_1691 https://fairsharing.org/FAIRsharing.p77ph9) /LinkedData.

 > :warning: This YAML document must be a UTF-8 text encoded, JSON (URL_TO_INSERT_RECORD-ABBREV_1692 https://fairsharing.org/FAIRsharing.5bbab9) -compatible subset of YAML in order to be processed by the SALAD schema processor.
...
`source`: https://github.com (URL_TO_INSERT_RECORD-HOMEPAGE_1693 https://fairsharing.org/FAIRsharing.c55d5e) /arvados/bh20-seq-resource/blob/master/bh20sequploader/bh20seq-schema.yml
...
`source`: https://github.com (URL_TO_INSERT_RECORD-HOMEPAGE_1694 https://fairsharing.org/FAIRsharing.c55d5e) /arvados/bh20-seq-resource/blob/master/example/maximum_metadata_example.yaml

## 3. Conversion from YAML to RDF:

Using the schema SALAD python package, the YAML instance file can be easily converted to RDF (URL_TO_INSERT_RECORD-ABBREV_1695 https://fairsharing.org/FAIRsharing.p77ph9)  as shown in the code snippet below:

```bash
$ pip install schema_salad
```
Get JSON (URL_TO_INSERT_RECORD-ABBREV_1696 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_1697 https://fairsharing.org/FAIRsharing.8f9bbb)  context::
```bash
$ schema-salad-tool --print-jsonld-context myschema.yml mydocument.yml
```

Convert a document to JSON (URL_TO_INSERT_RECORD-ABBREV_1698 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_1699 https://fairsharing.org/FAIRsharing.8f9bbb) ::
```bash
...
ShEx stands for `Shape Expression` and is a syntax for validating and describing RDF (URL_TO_INSERT_RECORD-ABBREV_1700 https://fairsharing.org/FAIRsharing.p77ph9)  graphs. ShEx expressions can be used both to describe RDF (URL_TO_INSERT_RECORD-ABBREV_1701 https://fairsharing.org/FAIRsharing.p77ph9)  and check the conformance of RDF (URL_TO_INSERT_RECORD-ABBREV_1702 https://fairsharing.org/FAIRsharing.p77ph9)  data. The ShEx language  specification was published by the W3C Shape Expressions Community Group but it is not a W3C Standard nor is it on the W3C Standards Track.

It should be noted that the current W3C Technical Recommendations for RDF (URL_TO_INSERT_RECORD-ABBREV_1703 https://fairsharing.org/FAIRsharing.p77ph9)  shape validation is the [SHACL specification](https://www.w3.org/TR/shacl/).
...
While defining a SALAD schema using YAML allows to list key entities and their attributes, it does not allow to check constraints. This has to be done on the RDF (URL_TO_INSERT_RECORD-ABBREV_1704 https://fairsharing.org/FAIRsharing.p77ph9)  which needs to be checks for compliancee against a set of constraints which can be expressed using ShEx. Working with a ShEx expert ([Dr Jose Emilio Labra Gayo - (Oviedo Uni)](http://di002.edv.uniovi.es/~labra/), the following [Shape Expression syntax](https://shex.io/shex-semantics/) profile was developed and used to validate the RDF (URL_TO_INSERT_RECORD-ABBREV_1705 https://fairsharing.org/FAIRsharing.p77ph9)  before persistence to the SPARQL (URL_TO_INSERT_RECORD-ABBREV_1706 https://fairsharing.org/FAIRsharing.87ccfd)  endpoint.
...
`source:` https://github.com (URL_TO_INSERT_RECORD-HOMEPAGE_1707 https://fairsharing.org/FAIRsharing.c55d5e) /arvados/bh20-seq-resource/blob/master/bh20sequploader/bh20seq-shex.rdf

Using the [WESO](http://www.weso.es/) developed [RDF (URL_TO_INSERT_RECORD-ABBREV_1708 https://fairsharing.org/FAIRsharing.p77ph9)  shape viewer](http://rdfshape.weso.es/),  Shape Expression can be rendered graphical. In the example below a [schema.org](https://schema.org) base shex expression in presented.
...
The following endpoint during the Elixir Covid-19 Biohackthon and metadata information converted from the YAML definition to RDF (URL_TO_INSERT_RECORD-ABBREV_1709 https://fairsharing.org/FAIRsharing.p77ph9)  turtle format was loaded in the following SPARQL (URL_TO_INSERT_RECORD-ABBREV_1710 https://fairsharing.org/FAIRsharing.87ccfd)  Endpoint.
...
In this recipe, we have presented how to implement a minimal medata profile and validate data entry with a specific technology stack: namely using RDF (URL_TO_INSERT_RECORD-ABBREV_1711 https://fairsharing.org/FAIRsharing.p77ph9)  and Shape Express standard. 
...
This recipe tackles an important aspect of the FAIR (URL_TO_INSERT_RECORD-ABBREV_1713 https://fairsharing.org/FAIRsharing.WWI10U)  principles (URL_TO_INSERT_RECORD-NAME_1712 https://fairsharing.org/FAIRsharing.WWI10U) , shining the light on the need to provide sufficient descriptive metadata to associate with an assay data file to allow its correct interpretation.
The recipe therefore provides a piece of the jigsaw to establish a FAIR (URL_TO_INSERT_RECORD-ABBREV_1714 https://fairsharing.org/FAIRsharing.WWI10U)  datasets.
There are some caveats or improvements which could be made. For instance, the devised shex expression and the associated instance RDF (URL_TO_INSERT_RECORD-ABBREV_1715 https://fairsharing.org/FAIRsharing.p77ph9)  graph  could be assigned a persistent identifiers (PID).
Another improvement could be a better integration with repositories such FAIRsharing (URL_TO_INSERT_RECORD-NAME_1716 https://fairsharing.org/FAIRsharing.2abjs5)  or the main sequence data submission systems such as INSDC deposition pipelines.
