...
The purpose of this recipe is to show how to create a metadata collection (URL_TO_INSERT_TERM_3212 https://fairsharing.org/search?recordType=collection)  form complying with a community minimal informat (URL_TO_INSERT_TERM_3213 https://fairsharing.org/search?recordType=model_and_format) ion checklist (MIUViG (URL_TO_INSERT_RECORD-ABBREV_3214 https://fairsharing.org/FAIRsharing.bd9566) ), in the context of Covid-19 strain sequencing assays carried on patient collected samples.
In addition, the recipe includes the conversion of sample metadata to an RDF (URL_TO_INSERT_RECORD-ABBREV_3215 https://fairsharing.org/FAIRsharing.p77ph9) /Linked Data graph and checks its structure for conformance to requirement using the ShapeExpression specifications.
Finally, use queries expressed in SPARQL (URL_TO_INSERT_RECORD-ABBREV_3216 https://fairsharing.org/FAIRsharing.87ccfd)  are shown to demonstrate potential data integration scenarios.
...
| Data Format (URL_TO_INSERT_TERM_3218 https://fairsharing.org/search?recordType=model_and_format) s  | Terminologies (URL_TO_INSERT_TERM_3219 https://fairsharing.org/search?recordType=terminology_artefact)  | Model (URL_TO_INSERT_TERM_3217 https://fairsharing.org/search?recordType=model_and_format) s  |
| :------------- | :------------- | :------------- |
...
|:--|:--|
| [Semantic Annotation for Linked Arvado Data (SALAD)](https://www.commonwl.org/draft-3/SchemaSalad.html)  | conversion from  YAML to RDF (URL_TO_INSERT_RECORD-ABBREV_3220 https://fairsharing.org/FAIRsharing.p77ph9)  |
|[WESO shExVisualize](http://rdfshape.weso.es/shExVisualize) | Shape expression syntax visualization |
| [Virtuoso](https://virtuoso.openlinksw.com/) | RDF (URL_TO_INSERT_RECORD-ABBREV_3221 https://fairsharing.org/FAIRsharing.p77ph9)  triple store |



## Introduction

>:informat (URL_TO_INSERT_TERM_3222 https://fairsharing.org/search?recordType=model_and_format) ion_source: 
This recipe is adapted from work carried during the Elixir Covid-19 biohackathon, by the ontology (URL_TO_INSERT_TERM_3223 https://fairsharing.org/search?recordType=terminology_artefact)  and workflow tracks and presented [here](http://covid19.genenetwork.org/) and detailed in the [following manuscript](https://github.com/arvados/bh20-seq-resource/blob/master/paper/paper.md) while all the code and associated material is hosted on this [github (URL_TO_INSERT_RECORD-NAME_3224 https://fairsharing.org/FAIRsharing.c55d5e)  repository](https://github.com/arvados/bh20-seq-resource).


>:informat (URL_TO_INSERT_TERM_3225 https://fairsharing.org/search?recordType=model_and_format) ion_source: 
...
> * Expressing instance data in RDF (URL_TO_INSERT_RECORD-ABBREV_3228 https://fairsharing.org/FAIRsharing.p77ph9) /linked data format (URL_TO_INSERT_TERM_3227 https://fairsharing.org/search?recordType=model_and_format) 
> * Validating RDF (URL_TO_INSERT_RECORD-ABBREV_3229 https://fairsharing.org/FAIRsharing.p77ph9)  instance data against requirements using a Shape Expression(SHEX)
> * Testing query cases by formulating SPARQL (URL_TO_INSERT_RECORD-ABBREV_3230 https://fairsharing.org/FAIRsharing.87ccfd)  queries
...
<br>Based on the [Genome Standards Consortium](https://gensc.org/) metadata requirement profile for uncultivated viral sample, also known as the [Minimum Informat (URL_TO_INSERT_TERM_3232 https://fairsharing.org/search?recordType=model_and_format) ion About an Uncultivated Virus Genome (URL_TO_INSERT_RECORD-NAME_3233 https://fairsharing.org/FAIRsharing.bd9566)  (MIUViG (URL_TO_INSERT_RECORD-ABBREV_3234 https://fairsharing.org/FAIRsharing.bd9566) )](https://github.com/GenomicsStandardsConsortium/mixs/blob/master/UViGs_MIxS.xlsx), the first step is to anchor the tags defined by GSC (URL_TO_INSERT_RECORD-ABBREV_3235 https://fairsharing.org/3528)  and approved by the those **International Nucleotide Sequence Database (URL_TO_INSERT_TERM_3231 https://fairsharing.org/search?fairsharingRegistry=Database)  Collaboration** ([INSDC](http://www.insdc.org/)) tags to one (or more) semantic framework(s).
...
However, for the final implementation, only the OBO (URL_TO_INSERT_RECORD-ABBREV_3236 https://fairsharing.org/FAIRsharing.847069)  related mappings have been used as show in the following figure.
...
Quoting the project (URL_TO_INSERT_TERM_3237 https://fairsharing.org/search?recordType=project) 's documentation, "*[the Semantic Annotations for Linked Avro Data (SALAD)](https://github.com/common-workflow-language/schema_salad) is a schema language for describing JSON (URL_TO_INSERT_RECORD-ABBREV_3242 https://fairsharing.org/FAIRsharing.5bbab9)  or YAML structured linked data documents. `SALAD schema` describes rules for preprocessing, structural validation, and hyperlink checking for documents described by a Salad schema. Salad supports rich data model (URL_TO_INSERT_TERM_3238 https://fairsharing.org/search?recordType=model_and_format) ing with inheritance, template specialization, object identifier (URL_TO_INSERT_TERM_3241 https://fairsharing.org/search?recordType=identifier_schema) s, object references, documentation generation, code generation, and transformat (URL_TO_INSERT_TERM_3240 https://fairsharing.org/search?recordType=model_and_format) ion to RDF (URL_TO_INSERT_RECORD-ABBREV_3243 https://fairsharing.org/FAIRsharing.p77ph9) . SALAD provides a bridge between document and record oriented data model (URL_TO_INSERT_TERM_3239 https://fairsharing.org/search?recordType=model_and_format) ing and the Semantic Web."*
...
But in this example, we are using the SALAD schema to capture the annotation requirements in a [YAML](https://yaml.org) document, while also embedding the semantics constraints, which can then be used to  to build a web form (see below) but also support conversion to RDF (URL_TO_INSERT_RECORD-ABBREV_3244 https://fairsharing.org/FAIRsharing.p77ph9) /LinkedData.

 > :warning: This YAML document must be a UTF-8 text encoded, JSON (URL_TO_INSERT_RECORD-ABBREV_3245 https://fairsharing.org/FAIRsharing.5bbab9) -compatible subset of YAML in order to be processed by the SALAD schema processor.


Below is a partial view of the YAML defined metadata  form, showing how `host` informat (URL_TO_INSERT_TERM_3246 https://fairsharing.org/search?recordType=model_and_format) ion requirements have been defined:
...
`source`: https://github.com (URL_TO_INSERT_RECORD-HOMEPAGE_3247 https://fairsharing.org/FAIRsharing.c55d5e) /arvados/bh20-seq-resource/blob/master/bh20sequploader/bh20seq-schema.yml
...
When users submit informat (URL_TO_INSERT_TERM_3248 https://fairsharing.org/search?recordType=model_and_format) ion via the form (or by other programatic means), an instance YAML file is generated, which looks like this:
...
`source`: https://github.com (URL_TO_INSERT_RECORD-HOMEPAGE_3249 https://fairsharing.org/FAIRsharing.c55d5e) /arvados/bh20-seq-resource/blob/master/example/maximum_metadata_example.yaml

## 3. Conversion from YAML to RDF:

Using the schema SALAD python package, the YAML instance file can be easily converted to RDF (URL_TO_INSERT_RECORD-ABBREV_3250 https://fairsharing.org/FAIRsharing.p77ph9)  as shown in the code snippet below:

```bash
$ pip install schema_salad
```
Get JSON (URL_TO_INSERT_RECORD-ABBREV_3251 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_3252 https://fairsharing.org/FAIRsharing.8f9bbb)  context::
```bash
$ schema-salad-tool --print-jsonld-context myschema.yml mydocument.yml
```

Convert a document to JSON (URL_TO_INSERT_RECORD-ABBREV_3253 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_3254 https://fairsharing.org/FAIRsharing.8f9bbb) ::
```bash
...
ShEx stands for `Shape Expression` and is a syntax for validating and describing RDF (URL_TO_INSERT_RECORD-ABBREV_3257 https://fairsharing.org/FAIRsharing.p77ph9)  graphs. ShEx expressions can be used both to describe RDF (URL_TO_INSERT_RECORD-ABBREV_3258 https://fairsharing.org/FAIRsharing.p77ph9)  and check the conformance of RDF (URL_TO_INSERT_RECORD-ABBREV_3259 https://fairsharing.org/FAIRsharing.p77ph9)  data. The ShEx language  specification was published by the W3C Shape Expressions Community Group but it is not a W3C Standard (URL_TO_INSERT_TERM_3255 https://fairsharing.org/search?fairsharingRegistry=Standard)  nor is it on the W3C Standard (URL_TO_INSERT_TERM_3256 https://fairsharing.org/search?fairsharingRegistry=Standard) s Track.

It should be noted that the current W3C Technical Recommendations for RDF (URL_TO_INSERT_RECORD-ABBREV_3260 https://fairsharing.org/FAIRsharing.p77ph9)  shape validation is the [SHACL specification](https://www.w3.org/TR/shacl/).
...
While defining a SALAD schema using YAML allows to list key entities and their attributes, it does not allow to check constraints. This has to be done on the RDF (URL_TO_INSERT_RECORD-ABBREV_3261 https://fairsharing.org/FAIRsharing.p77ph9)  which needs to be checks for compliancee against a set of constraints which can be expressed using ShEx. Working with a ShEx expert ([Dr Jose Emilio Labra Gayo - (Oviedo Uni)](http://di002.edv.uniovi.es/~labra/), the following [Shape Expression syntax](https://shex.io/shex-semantics/) profile was developed and used to validate the RDF (URL_TO_INSERT_RECORD-ABBREV_3262 https://fairsharing.org/FAIRsharing.p77ph9)  before persistence to the SPARQL (URL_TO_INSERT_RECORD-ABBREV_3263 https://fairsharing.org/FAIRsharing.87ccfd)  endpoint.
...
`source:` https://github.com (URL_TO_INSERT_RECORD-HOMEPAGE_3264 https://fairsharing.org/FAIRsharing.c55d5e) /arvados/bh20-seq-resource/blob/master/bh20sequploader/bh20seq-shex.rdf

Using the [WESO](http://www.weso.es/) developed [RDF (URL_TO_INSERT_RECORD-ABBREV_3265 https://fairsharing.org/FAIRsharing.p77ph9)  shape viewer](http://rdfshape.weso.es/),  Shape Expression can be rendered graphical. In the example below a [schema.org](https://schema.org) base shex expression in presented.
...
The following endpoint during the Elixir Covid-19 Biohackthon and metadata informat (URL_TO_INSERT_TERM_3266 https://fairsharing.org/search?recordType=model_and_format) ion converted from the YAML definition to RDF (URL_TO_INSERT_RECORD-ABBREV_3268 https://fairsharing.org/FAIRsharing.p77ph9)  turtle format (URL_TO_INSERT_TERM_3267 https://fairsharing.org/search?recordType=model_and_format)  was loaded in the following SPARQL (URL_TO_INSERT_RECORD-ABBREV_3269 https://fairsharing.org/FAIRsharing.87ccfd)  Endpoint.

http://sparql.genenetwork.org/sparql/

The collection (URL_TO_INSERT_TERM_3270 https://fairsharing.org/search?recordType=collection)  of metadata in rdf format (URL_TO_INSERT_TERM_3271 https://fairsharing.org/search?recordType=model_and_format)  is available for download: https://collections.lugli.arvadosapi.com/c=lugli-4zz18-z513nlpqm03hpca/mergedmetadata.ttl
...
In this recipe, we have presented how to implement a minimal medata profile and validate data entry with a specific technology stack: namely using RDF (URL_TO_INSERT_RECORD-ABBREV_3273 https://fairsharing.org/FAIRsharing.p77ph9)  and Shape Express standard (URL_TO_INSERT_TERM_3272 https://fairsharing.org/search?fairsharingRegistry=Standard) . 
...
This recipe tackles an important aspect of the FAIR (URL_TO_INSERT_RECORD-ABBREV_3275 https://fairsharing.org/FAIRsharing.WWI10U)  principles (URL_TO_INSERT_RECORD-NAME_3274 https://fairsharing.org/FAIRsharing.WWI10U) , shining the light on the need to provide sufficient descriptive metadata to associate with an assay data file to allow its correct interpretation.
The recipe therefore provides a piece of the jigsaw to establish a FAIR (URL_TO_INSERT_RECORD-ABBREV_3276 https://fairsharing.org/FAIRsharing.WWI10U)  datasets.
There are some caveats or improvements which could be made. For instance, the devised shex expression and the associated instance RDF (URL_TO_INSERT_RECORD-ABBREV_3278 https://fairsharing.org/FAIRsharing.p77ph9)  graph  could be assigned a persistent identifier (URL_TO_INSERT_TERM_3277 https://fairsharing.org/search?recordType=identifier_schema) s (PID).
Another improvement could be a better integration with repositories (URL_TO_INSERT_TERM_3279 https://fairsharing.org/search?recordType=repository)  such FAIRsharing (URL_TO_INSERT_RECORD-NAME_3280 https://fairsharing.org/FAIRsharing.2abjs5)  or the main sequence data submission systems such as INSDC deposition pipelines.
