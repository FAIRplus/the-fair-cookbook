...
To create a **persistent**, **globally unique** and **resolvable identifier (URL_TO_INSERT_TERM_1081 https://fairsharing.org/search?recordType=identifier_schema) ** using the ***Minid client*** accessing the Minid 2.0 release {footcite}`Madduri2019` {footcite}`minid-identifier (URL_TO_INSERT_TERM_1082 https://fairsharing.org/search?recordType=identifier_schema) s`.
...
height: 550px
name: Minting Minid
alt: An overview of the identifier (URL_TO_INSERT_TERM_1083 https://fairsharing.org/search?recordType=identifier_schema)  minting process
---
An overview of the identifier (URL_TO_INSERT_TERM_1084 https://fairsharing.org/search?recordType=identifier_schema)  minting process.
```
````
...
1. prerequisite: create a minig-config.cfg file
  ...
  As a convenience you need specify this informat (URL_TO_INSERT_TERM_1085 https://fairsharing.org/search?recordType=model_and_format) ion in a minid configuration file (`~/.minid/minid-config.cfg`)
...
* Check a known minid identifier (URL_TO_INSERT_TERM_1086 https://fairsharing.org/search?recordType=identifier_schema) 
...
* Create a new identifier (URL_TO_INSERT_TERM_1087 https://fairsharing.org/search?recordType=identifier_schema)  (the `--location` option, if provided, must be at the end).

```bash
$ minid --register [--title <title>] <file_name> [--locations <loc1>..<locN>]
```

* Update metadata about an identifier (URL_TO_INSERT_TERM_1088 https://fairsharing.org/search?recordType=identifier_schema) :
...
Minids can only be assigned to a single file. In order to assign a minid to a collection (URL_TO_INSERT_TERM_1089 https://fairsharing.org/search?recordType=collection)  of files, we recommend using a `BDBag <https://github.com (URL_TO_INSERT_RECORD-HOMEPAGE_1091 https://fairsharing.org/FAIRsharing.c55d5e) /ini-bdds/bdbag>`_ or the minid file manifest format (URL_TO_INSERT_TERM_1090 https://fairsharing.org/search?recordType=model_and_format) .

The minid file manifest format (URL_TO_INSERT_TERM_1092 https://fairsharing.org/search?recordType=model_and_format)  is a JSON (URL_TO_INSERT_RECORD-ABBREV_1094 https://fairsharing.org/FAIRsharing.5bbab9) -based format (URL_TO_INSERT_TERM_1093 https://fairsharing.org/search?recordType=model_and_format)  that enumerates a list of files as JSON (URL_TO_INSERT_RECORD-ABBREV_1095 https://fairsharing.org/FAIRsharing.5bbab9)  objects that have the following attributes:
...
* url: the URL (URL_TO_INSERT_RECORD-ABBREV_1096 https://fairsharing.org/FAIRsharing.9d38e2)  to the file.

The manifest may be used to create a minid for a collection (URL_TO_INSERT_TERM_1097 https://fairsharing.org/search?recordType=collection)  of files or alternatively as input to the minid batch-register command.
