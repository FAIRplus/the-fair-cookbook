...
to confirm that the actual content, i.e. the bits that make up the informat (URL_TO_INSERT_TERM_2288 https://fairsharing.org/search?recordType=model_and_format) ion, arrived correctly in the target system.
  ...
  - Start up a console. Type 
    ...
    and hit return.
      - You should see as output: 
        ```text
        md5sum (GNU core (URL_TO_INSERT_RECORD_2289 https://fairsharing.org/FAIRsharing.m283c)  (URL_TO_INSERT_RECORD_2290 https://fairsharing.org/FAIRsharing.xMmOCL) utils) 8.30
...
For 60 pictures of 3.5 MB (URL_TO_INSERT_RECORD_2292 https://fairsharing.org/FAIRsharing.a1rp4c)  each, this command took 1.6 seconds on a MacBook (URL_TO_INSERT_RECORD_2291 https://fairsharing.org/FAIRsharing.cbz72b)  Pro (2017) with a SSD drive.
...
This recipe in its current form has the following limitations:
  ...
  - the above assumes that you don't have a problem with calculating the checksums sequentially. Depending on your system's resources (especially available CP (URL_TO_INSERT_RECORD_2293 https://fairsharing.org/FAIRsharing.wP3t2L) U time), this calculation of checksums might take a while, however. A common benchmark on a typical laptop is: 0.01 seconds per MB (URL_TO_INSERT_RECORD_2294 https://fairsharing.org/FAIRsharing.a1rp4c)  of data.  
  - you should mind the general limitations of checksums, which are however not covered in this recipe.
  - there is a known clash between the output format (URL_TO_INSERT_TERM_2296 https://fairsharing.org/search?recordType=model_and_format)  of the GNU / Linux tool `md5sum` and the macOS / BSD tool `md5`. Their standard (URL_TO_INSERT_TERM_2295 https://fairsharing.org/search?fairsharingRegistry=Standard)  output format (URL_TO_INSERT_TERM_2297 https://fairsharing.org/search?recordType=model_and_format) s are incompatible; combining a macOS-based system with a Linux-based system, either one as source or target, is therefore not straightforward. (hint: the Linux tool has the `--tag` flag which generates macOS-compatible output.) 
...
- The tool above could be used to calculate checksums in parallel if typical scheduling systems and multiple worker nodes are available sharing the same file system (equivalently, this would be possible in a cloud arch (URL_TO_INSERT_RECORD_2298 https://fairsharing.org/FAIRsharing.52b22c) itecture).
...
- Wikiped (URL_TO_INSERT_RECORD_2299 https://fairsharing.org/FAIRsharing.31385c) ia article on checksums: <https://en.wikipedia.org/wiki/Checksum>
- Wikiped (URL_TO_INSERT_RECORD_2300 https://fairsharing.org/FAIRsharing.31385c) ia article on the algorithm `md5`: <https://en.wikipedia.org/wiki/MD5>
- Wikiped (URL_TO_INSERT_RECORD_2301 https://fairsharing.org/FAIRsharing.31385c) ia article on the tool `md5sum`: <https://en.wikipedia.org/wiki/Md5sum>
