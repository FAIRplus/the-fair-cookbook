...
The **FAIR (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.WWI10U)  principles**, under the `Findability` and the `Accessibility` chapters respectively, state that: 
...
``` 
 ...
 - the EMB (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.a1rp4c) L-EBI
...
All these key points will be developed (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.31385c)  in this recipe.
...
| HTTP<!-- TO (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.w69t6r) DO add a link to corresponding document --> |  |  |
| RDF (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.p77ph9) <!-- TO (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.w69t6r) DO add a link to corresponding document --> |  |  |
...
#### Generation in Python
The following code snippet shows the generation of a hash for a string (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9b7wvk)  using the Python hashlib package:
...
In our context, the use of the hashing function is to generate a unique key which may be used to generate a URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) . This simply indicates a technical option for generating opaque URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) , not that it is necessarily the most widespread approach.
...
In other words, in order to create globally unique identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s `for the web`, it is necessary to understand what Uniform Resource Locator (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) s<!-- TO (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.w69t6r) DO add a link to corresponding document --> (a.k.a `URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) `) are and how to construct them for use with the Hypertext Transfer Protocol<!-- TO (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.w69t6r) DO add a link to corresponding document -->.

This results in URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) s of the following form
...
The structure of `URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) `, according to the World Wide Web Consortium (W3C) specification, is as follows:
...
The most relevant `URI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1)  scheme` in the context of FAIR (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.WWI10U)  data and Linked Open Data are `http` and `https` which denote the `Hypertext Transfer Protocol` and the `Hypertext Tranfer Protocol Secure`.

### `authority`:

Besides setting the `scheme`, the other essential fragment of a URI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1)  is the `authority`, which according to the Internet Engineering Task Force (IETF) specifications, presents the following characteristics:
```
...
In the `authority`, the notion of `host` corresponds to the `Internet Protocol (IP) address` of a server hosting (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.q7bkqr)  a resource. Often, the IP address corresponding to the `host` is given a `host name` such as `www.example.org`. The `host name` should be a `Qualified Domain Name` at minima, or a `Fully Qualified Domain Name (FQDN)` ideally and registered with the `Domain Name Service (DNS)` which allows the resolution (lookup) between the `ip address` and the `hostname`.
...
### `query`:
The `query` is an optional part of the URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  syntax that starts with a `?`. Typically the `query` component consists of a service of key-value pairs separated by an `&` deliminator.

In the context resolvable identiers, `query` components should be avoided.

### `fragment`
The `fragement` is an optional part of the URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  syntax that starts with a `#`. It identifies a component within the returned resource and is used for client side processing, e.g. to scroll to a particular section within a webpage.

## Generating Resolvable URLs

In the context of FAIR (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.WWI10U)  data, resources on the web must have **unique**, **persistent**, and **resolvable** identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s.
In order to achieve the capability of `persistence`, it is necessary for the resource identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s to comply to the RFC 3986 IETF standard (URL_TO_INSERT_TERM https://fairsharing.org/search?fairsharingRegistry=Standard)  for URI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) s (and IRIs, which are URI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1)  extended to cope with unicode). This means that it must comprise the following components:
...
- UniProt P38398 web page IRI: https://www.uniprot.org (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.s1ne3g) /uniprot/P38398
- UniProt protein P38398 IRI: http://purl.uniprot.org/uniprot/P38398

Once such URI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) s are available, one may also turn them into compact identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s called CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) s. This will be discussed further in the next section.

## Identifier Resolution - Enabling persistence through indirection


This relates to the following FAIR (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.WWI10U)  principle mentioned in the introduction:
...
A PURL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.3e603c)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  is a **`persistent URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) `**, meaning that it provides a **permanent address to access a resource on the web**.

To understand the notion of PURL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.3e603c)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) , one needs to first get fam (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d0886a) iliar with the notion of `url indirection` (also known as `url redirect` or `url forwarding` ), which refers to the practice of providing a stable, fixed web address/url, but setting it up so that it points to another content, which may be periodically modified. 

When a user retrieves a PURL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.3e603c)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) , they will be **`redirected`** to the current location of the resource.
When an author needs to move a page, they can update the PURL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.3e603c)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  to point to the new location.
...
PURL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.3e603c)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) s with a `common prefix` are grouped (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.31385c)  together into **domains**. Each domain has a single maintainer who can add new PURL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.3e603c)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) s to the domain and make changes to existing (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.q7bkqr)  PURL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.3e603c)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) s within the domain.


FAIR (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.WWI10U)  Principle A1 states that:
...
When the identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema)  is not a resolvable URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) , then `Identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema)  Resolution Services` are required that know how to map (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.53edcc)  an IRI to a location for the data. 



### Introducing CURIES or Compact URIs
CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) s (short for compact URI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) s) are defined by a World Wide Web Consortium Working Group Note [CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db)  Syntax 1.0](https://www.w3.org/TR/2010/NOTE-curie-20101216 (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) /), and provide a human readable shortening of IRIs. 

The CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db)  consists of a **`namespace prefix`** followed by the **`local identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) `**.

There are some widely used and defined CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) s such as DOI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.hFLKCn) s and ISBN numbers. For example the DOI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.hFLKCn)  `[doi:10.1038/sdata.2016.18]` refers to the FAIR (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.WWI10U)  Principles (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.WWI10U)  paper. The Digital Object Identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.hFLKCn)  System web site (https://www.doi.org (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.hFLKCn) /) provides a resolution service for DOI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.hFLKCn) s. The service is available as a web form on the site or can be used by appending a DOI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.hFLKCn)  to the website.The client will be redirected to the URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  where the resource about the concept is located, e.g. for the FAIR (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.WWI10U)  Data Principles paper we can use the URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  https://www.doi.org (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.hFLKCn) /10.1038/sdata.2016.18 to resolve the paper's DOI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.hFLKCn) . This results in the client being taken to the page at https://www.nature.com/articles/sdata201618.

`Namespaces` can be defined *by convention*, such as the case with `doi`, and registered with services to allow for the resolution of CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) s (see [Identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema)  Resolution Services](##identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) -resolution-services) below). These are extensively used to map (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.53edcc)  CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) s to URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) s that can be resolved.

Going back to our *Life Science context*, we can use the following CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db)  `[uniprot:P38398]` to refer to the UniProt record for the protein. 
...
[^safe-CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) ]: The CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) S are included in square brackets to make them *safe CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) s*, meaning that they should not be confused for URI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) s.
...
* [purl.org](https://archive.org/services/purl/)
    ...
    > The PURL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.3e603c)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  system is a service of the Internet Arch (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.52b22c) ive, which provides an interface to administer domain. For more informat (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=model_and_format) ion about the service, visit https://archive.org/services/purl/help
    >

* [w3id (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.S6BoUk) s](https://w3id.org (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.S6BoUk) /)
    
    > Permanent Identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s for the Web. Secure, permanent URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) s for your Web application that will stand the test of time.
    > - authority registration service
    > - resolution service
    > - redirection service:
    > 
    > Send a request to add a redirect to the public-perma-id@w3.org mailing list. Make sure to include the URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  that you want on w3id (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.S6BoUk) .org (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.S6BoUk) , the URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  that you want to redirect to, and the HTTP code that you want to use when redirecting. An administrator will then create the redirect for you.


* [Identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s.org](http://identifiers.org/)

    > [Identifiers.org](https://identifiers.org) is a **Resolution Service** provides consistent access to life science data using [`Compact Uniform Resource Identifier (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) s`](https://www.w3.org/TR/2010/NOTE-curie-20101216 (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db) /), hosted by the EBI provides a resolution service, both as a web form and through the URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  pattern {footcite}`Juty2012`. 
    ...
    > Datasets can register their *namespace `prefix`* together with their `identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema)  pattern`. The service can then be used in the same way as the DOI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.hFLKCn)  resolution service. So for the UniProt page about BRCA1, we can resolve the CURI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) E (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.af21db)  `[uniprot:P38938]` using Identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s.org. This means that the URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  https://identifiers.org/uniprot:P38938 resolves to the UniProt page https://www.uniprot.org (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.s1ne3g) /uniprot/P38938.



* [Name2Things](https://n2t.net/)

    > [Name2Things](https://n2t.net/) (N2T) is a **Resolution Service**, maintained at the California Digital Library (CDL) within the University of California (UC) Office of the President. CDL supports electronic library services for ten UC campuses and affiliated law schools, medical centers, and national laboratories, as well as hundreds of museums, herbaria, botanical gardens, etc. Similar to URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  shorteners like bit.ly, N2T serves content **indirectly**.
...
* [Bioregistry (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.250a8c) ](https://bioregistry.io (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.250a8c) /)

    > [Bioregistry](https://bioregistry.io (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.250a8c) /) is a **Resolution Service**, developed (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.31385c)  in a [GitHub (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.c55d5e)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.c55d5e)  repository](https://github.com (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.c55d5e) /biopragmatics/bioregistry). Like Identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s.org it has a registry, but also a registry of registries, and it imports data from Identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s.org and Name-to-Thing but extends beyond identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s for things but also supports, for example, ontologies (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=terminology_artefact) . As a community effort, new namespace prefixes and their identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema)  patterns can be [registered via GitHub (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.c55d5e)  (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.c55d5e)  issues](https://github.com (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.c55d5e) /biopragmatics/bioregistry/issues/new/choose). Compact identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s are supported and the URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2)  https://bioregistry.io (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.250a8c) /chebi:138488 resolves to the ChEBI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.62qk8w)  page https://www.ebi.ac.uk/chebi (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.62qk8w) /searchId.do?chebiId=CHEBI:138488. Bioregistry (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.250a8c)  provides an [API to query the registry](https://bioregistry.io (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.250a8c) /apidocs/) itself.
...
In this recipe, we have given an overview of globally unique and persistent identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema)  {footcite}`McMurry2017`, {footcite}`Ananthakrishnan2020`, i.e. FAIR (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.WWI10U)  principle F1. We have covered:
...
But we can not conclude this section on persistent identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) s without stressing how central they are to the production of Linked Data or Linked Open Data, which rely on 3 W3C standard (URL_TO_INSERT_TERM https://fairsharing.org/search?fairsharingRegistry=Standard) s: URI (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.d261e1) {footcite}`URL (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.9d38e2) `,{footcite}`cool-uri`,{footcite}`cool-urisemweb`, RDF (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.p77ph9) {footcite}`RDF (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.p77ph9) concepts` and HTTP.
...
* [Identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema)  Map (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.53edcc) ping Services](fcb-identifier (URL_TO_INSERT_TERM https://fairsharing.org/search?recordType=identifier_schema) -map (URL_TO_INSERT_RECORD https://fairsharing.org/FAIRsharing.53edcc) ping)
