...
Data Enclave
	a data resource the content of which can only be accessed by being physically present on the site where the data is held and no connection to the outside world and the web is possible. This model is used to safeguard extremely sensitive data by drastically controlling who accesses the information.
...
Descriptive Metadata
	metadata the aim of which is to allow provision of domain specific content to make is available to agents, human or machines.
```
...
GUPRID
	Globally Unique Persistent Resolvable Identifier, a type of persistent identifier with added properties such as guaranteed uniqueness and accessibility via HTTP protocol.
```
...
```{glossary}
MTA
	a Material Transfer Agreement is a legal document stipulating the conditions under which physical entities / properties may be transferred between organizations signatories of the agreement.
