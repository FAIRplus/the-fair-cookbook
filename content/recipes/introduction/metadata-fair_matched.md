...
> To introduce the notion of metadata and detail why it matters in the context of FAIR (URL_TO_INSERT_RECORD_6638 https://fairsharing.org/FAIRsharing.WWI10U) . 
...
Upon reading the [FAIR principles](fcb-fair-principles), one can't help but notice that everything rests on the availability of **machine readable metadata**. For a number of newcomers to FAIR (URL_TO_INSERT_RECORD_6639 https://fairsharing.org/FAIRsharing.WWI10U)  or Data Management for that matter, the first hurdl (URL_TO_INSERT_RECORD_6640 https://fairsharing.org/FAIRsharing.vm4688) e is to grasp the notion of 'metadata'.
Below are a number of commonly found definitions of metadata:

* The recursive definition **"data about the data"**, which provides the idea that to understand data, you need extra data describing it {footcite}`wikidata (URL_TO_INSERT_RECORD_6641 https://fairsharing.org/FAIRsharing.6s749p) Metadata`.
...
The definitions above hints that *metadata* is indeed important and above all pervasive in any modern society (URL_TO_INSERT_TERM_6642 https://fairsharing.org/search?recordType=society)  which relies on informat (URL_TO_INSERT_TERM_6643 https://fairsharing.org/search?recordType=model_and_format) ion technology.
...
```bash
@Article{Alter2020DATS (URL_TO_INSERT_RECORD_6644 https://fairsharing.org/FAIRsharing.e20vsd) ,
   Author="Alter, G.  and Gonzalez-Beltran, A.  and Ohno-Machado, L.  and Rocca-Serra, P. ",
   Title="{{T}he {D}ata {T}ags {S}uite ({D}{A}{T}{S}) model (URL_TO_INSERT_TERM_6645 https://fairsharing.org/search?recordType=model_and_format)  for discovering data access and use requirements}",
   Journal (URL_TO_INSERT_TERM_6646 https://fairsharing.org/search?recordType=journal) ="Gigascience",
   Year="2020",
   Volume="9",
   Number="2",
   Month="02"
