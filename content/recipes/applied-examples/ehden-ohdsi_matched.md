...
more Findable (notably the FAIR (URL_TO_INSERT_RECORD-ABBREV_24 https://fairsharing.org/FAIRsharing.WWI10U)  principles (URL_TO_INSERT_RECORD-NAME_23 https://fairsharing.org/FAIRsharing.WWI10U)  [F2](https://www.go-fair.org/fair-principles/f2-data-described-rich-metadata/)
...
*   [JSON-LD](https://www.w3.org/TR/json-ld11): JSON (URL_TO_INSERT_RECORD-ABBREV_25 https://fairsharing.org/FAIRsharing.5bbab9)  - Linked Data
...
*   [RDF](https://www.w3.org/TR/rdf-concepts): Resource Description Framework (URL_TO_INSERT_RECORD-NAME_26 https://fairsharing.org/FAIRsharing.p77ph9) 
...
on the Pistoia Alliance FAIR (URL_TO_INSERT_RECORD-ABBREV_27 https://fairsharing.org/FAIRsharing.WWI10U)  toolkit website, and also published in an EHDEN Deliverable, 
...
However, data standardization and harmonization does not necessarily equate with FAIR (URL_TO_INSERT_RECORD-ABBREV_28 https://fairsharing.org/FAIRsharing.WWI10U)  data 
...
not just data in the OMOP CDM (URL_TO_INSERT_RECORD-ABBREV_29 https://fairsharing.org/FAIRsharing.qk984b)  format.
...
From the FAIR (URL_TO_INSERT_RECORD-ABBREV_30 https://fairsharing.org/FAIRsharing.WWI10U)  assessment performed in order to better understand the current state of FAIRness of studies and databases
...
This recipe will describe a proof-of-concept that shows the step-by-step process of making resources more FAIR (URL_TO_INSERT_RECORD-ABBREV_31 https://fairsharing.org/FAIRsharing.WWI10U)  by:
...
*   Creating a website for FAIR (URL_TO_INSERT_RECORD-ABBREV_32 https://fairsharing.org/FAIRsharing.WWI10U)  dissemination of selected digital resources.
...
kind of digital resources reside in the OHDSI ecosystem. Digital resources in OHDSI include for example the OMOP CDM (URL_TO_INSERT_RECORD-ABBREV_35 https://fairsharing.org/FAIRsharing.qk984b)  
itself, the OMOP Standardized Vocabularies (URL_TO_INSERT_RECORD-ABBREV_36 https://fairsharing.org/FAIRsharing.658tcg) , studies, cohort definitions and many more. 
...
General sources that provide information on observational health databases that are conformed to the OMOP CDM (URL_TO_INSERT_RECORD-ABBREV_37 https://fairsharing.org/FAIRsharing.qk984b)  are the
...
The data network page of OHDSI shows some metadata on databases converted to the OMOP CDM (URL_TO_INSERT_RECORD-ABBREV_38 https://fairsharing.org/FAIRsharing.qk984b) .

The [EMIF catalogue](https://emif-catalogue.eu/) is developed by the European Medical Information Framework (EMIF) and
enables publishing and sharing of metadata on health data sources. Different consortia implementing the OMOP CDM (URL_TO_INSERT_RECORD-ABBREV_39 https://fairsharing.org/FAIRsharing.qk984b)  
...
To increase the findability we captured the structured and rich metadata, by developing a semantic model in RDF (URL_TO_INSERT_RECORD-ABBREV_40 https://fairsharing.org/FAIRsharing.p77ph9)  for the
...
Relevant Schema.org (URL_TO_INSERT_RECORD-NAME_46 https://fairsharing.org/FAIRsharing.hzdzq8)  types for studies and databases included: 
...
*   Author properties (e.g. GitHub (URL_TO_INSERT_RECORD-NAME_50 https://fairsharing.org/FAIRsharing.c55d5e)  handle (URL_TO_INSERT_RECORD-NAME_51 https://fairsharing.org/FAIRsharing.0b7e54)  and forum profile). 
...
The RDF (URL_TO_INSERT_RECORD-ABBREV_52 https://fairsharing.org/FAIRsharing.p77ph9)  model was validated using Apache Jena RIOT (version 3.15.0), using the following command:
...
The following figure shows the metadata in Turtle (URL_TO_INSERT_RECORD-ABBREV_57 https://fairsharing.org/FAIRsharing.3e194c)  and in YAML. YAML uses slightly different wording and syntax, 
...
The data in YAML gets converted into Turtle (URL_TO_INSERT_RECORD-ABBREV_58 https://fairsharing.org/FAIRsharing.3e194c)  programatically:
...
The Hugo backend that we created, reads in the study articles, including the YAML metadata, and produces an HTML (URL_TO_INSERT_RECORD-ABBREV_59 https://fairsharing.org/FAIRsharing.YugnuL)  output
from that which contains the article text but also embeds the entered metadata as JSON (URL_TO_INSERT_RECORD-ABBREV_60 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_61 https://fairsharing.org/FAIRsharing.8f9bbb)  
(see {numref}`ehden-ohdsi-figure3`). In order to do this, we have written custom processing logic that takes the YAML 
data as input and formats this into RDF (URL_TO_INSERT_RECORD-ABBREV_62 https://fairsharing.org/FAIRsharing.p77ph9)  in accordance with the metadata model.
...
vocabulary, because annotating metadata with Schema.org (URL_TO_INSERT_RECORD-NAME_68 https://fairsharing.org/FAIRsharing.hzdzq8)  types and properties gives meaning to the data. 
...
authors is displayed on this [website](https://covid19.ohdsi.app/), and is embedded as JSON (URL_TO_INSERT_RECORD-ABBREV_69 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_71 https://fairsharing.org/FAIRsharing.8f9bbb)  in the HTML (URL_TO_INSERT_RECORD-ABBREV_70 https://fairsharing.org/FAIRsharing.YugnuL)  code as well.
<!-- TODO: Give example of a page with markup -->

Embedding metadata in JSON (URL_TO_INSERT_RECORD-ABBREV_72 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_74 https://fairsharing.org/FAIRsharing.8f9bbb)  in HTML (URL_TO_INSERT_RECORD-ABBREV_73 https://fairsharing.org/FAIRsharing.YugnuL)  will enable web crawlers to scrape the metadata and will increase Findability 
by search engines. When for example querying 
[https://datasetsearch.research.google.com/](https://datasetsearch.research.google.com/),
the data should pop up when JSON (URL_TO_INSERT_RECORD-ABBREV_75 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_76 https://fairsharing.org/FAIRsharing.8f9bbb)  is embedded, but only when the `schema:Dataset` class is included
...
The generated JSON (URL_TO_INSERT_RECORD-ABBREV_77 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_78 https://fairsharing.org/FAIRsharing.8f9bbb)  files can be validated using [Google’s Rich Result Test](https://search.google.com/test/rich-results).
This tool can easily validate JSON (URL_TO_INSERT_RECORD-ABBREV_79 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_83 https://fairsharing.org/FAIRsharing.8f9bbb)  code, by either submitting a URL (URL_TO_INSERT_RECORD-ABBREV_82 https://fairsharing.org/FAIRsharing.9d38e2)  directing to the JSON (URL_TO_INSERT_RECORD-ABBREV_80 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_84 https://fairsharing.org/FAIRsharing.8f9bbb) , or by pasting JSON (URL_TO_INSERT_RECORD-ABBREV_81 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_85 https://fairsharing.org/FAIRsharing.8f9bbb)  code.
...
JSON (URL_TO_INSERT_RECORD-ABBREV_86 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_87 https://fairsharing.org/FAIRsharing.8f9bbb)  markup needed to mention the `schema:Dataset` class. In a first iteration, we had used our new 
`ohdsi:database` class and the website was not found (even if the `ohdsi:database is-a schema:Dataset` statement from
the model is repeated in the JSON (URL_TO_INSERT_RECORD-ABBREV_88 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_89 https://fairsharing.org/FAIRsharing.8f9bbb) ; 
Google's Schema.org (URL_TO_INSERT_RECORD-NAME_92 https://fairsharing.org/FAIRsharing.hzdzq8)  tools used a fixed context file, ignoring any additional statements included in your own JSON (URL_TO_INSERT_RECORD-ABBREV_90 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_91 https://fairsharing.org/FAIRsharing.8f9bbb) ).

To illustrate, the JSON (URL_TO_INSERT_RECORD-ABBREV_93 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_94 https://fairsharing.org/FAIRsharing.8f9bbb)  representation of metadata instantiation of the OHDSI Covid19 Icarius study looks like this in
JSON (URL_TO_INSERT_RECORD-ABBREV_95 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_97 https://fairsharing.org/FAIRsharing.8f9bbb)  (compare the metadata presented above in YAML and Turtle (URL_TO_INSERT_RECORD-ABBREV_96 https://fairsharing.org/FAIRsharing.3e194c)  format):
...
that use the YAML format, as the JSON (URL_TO_INSERT_RECORD-ABBREV_98 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_99 https://fairsharing.org/FAIRsharing.8f9bbb)  format is not the easiest format to enter data, to manually populate the static
...
The final result of applying all these steps looks like this: an HTML (URL_TO_INSERT_RECORD-ABBREV_104 https://fairsharing.org/FAIRsharing.YugnuL)  page in the browser with JSON (URL_TO_INSERT_RECORD-ABBREV_103 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_105 https://fairsharing.org/FAIRsharing.8f9bbb)  metadata under
the hood. 
A screenshot of the HTML (URL_TO_INSERT_RECORD-ABBREV_106 https://fairsharing.org/FAIRsharing.YugnuL)  page is presented in {numref}`ehden-ohdsi-figure6`.
...
In the HTML (URL_TO_INSERT_RECORD-ABBREV_108 https://fairsharing.org/FAIRsharing.YugnuL)  page of the record, the HTML (URL_TO_INSERT_RECORD-ABBREV_109 https://fairsharing.org/FAIRsharing.YugnuL)  head element is augmented with a script element which embeds the same JSON (URL_TO_INSERT_RECORD-ABBREV_107 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_110 https://fairsharing.org/FAIRsharing.8f9bbb) 
...
The key reusable element in this recipe is the use of Schema.org (URL_TO_INSERT_RECORD-NAME_113 https://fairsharing.org/FAIRsharing.hzdzq8)  JSON (URL_TO_INSERT_RECORD-ABBREV_111 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_112 https://fairsharing.org/FAIRsharing.8f9bbb)  statements in any website to promote
...
during the static website generation process to JSON (URL_TO_INSERT_RECORD-ABBREV_114 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_115 https://fairsharing.org/FAIRsharing.8f9bbb)  is quite laborious and also potentially introduces issues during
the conversion. Therefore, it is probably easier and more scalable to let the scientists write their metadata directly
in JSON (URL_TO_INSERT_RECORD-ABBREV_116 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_117 https://fairsharing.org/FAIRsharing.8f9bbb) , helped by some good examples and templates, and the Hugo software could even be extended to facilitate 
that as well.

Another next step that could be taken, besides publishing the JSON (URL_TO_INSERT_RECORD-ABBREV_118 https://fairsharing.org/FAIRsharing.5bbab9) -LD (URL_TO_INSERT_RECORD-ABBREV_119 https://fairsharing.org/FAIRsharing.8f9bbb)  metadata directly on the web, is to wrap it 
