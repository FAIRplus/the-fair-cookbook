...
This FAIRification pipeline converts the OncoTrack sample metadata to a structured and consistent data format (URL_TO_INSERT_TERM_632 https://fairsharing.org/search?recordType=model_and_format) , improves the findability, interoperability, and reusability of the metadata. 
...
To associate drug sensitivity data with related sample cohort metadata and convert them to machine-readable format (URL_TO_INSERT_TERM_642 https://fairsharing.org/search?recordType=model_and_format) s, we combined cohort metadata and drug sensitivity data to JSON (URL_TO_INSERT_RECORD-ABBREV_644 https://fairsharing.org/FAIRsharing.5bbab9)  files. The [BioSamples JSON schema](https://drive.google.com/file/d/1DoyOZ1uMFv0aPpAQUPkf7mS8vAoS6DCY/view) was used as the sample metadata template, allowing direct submissions to[ BioSamples database](https://www.ebi.ac.uk/biosamples/) and can be easily converted to other format (URL_TO_INSERT_TERM_643 https://fairsharing.org/search?recordType=model_and_format) s, like XML (URL_TO_INSERT_RECORD-ABBREV_645 https://fairsharing.org/FAIRsharing.b5cc91) , spreadsheets, etc.

The BioSamples JSON (URL_TO_INSERT_RECORD-ABBREV_647 https://fairsharing.org/FAIRsharing.5bbab9)  schema includes four blocks, submission metadata (e.g. submission domain, release date, update date), administrative metadata, (e.g. contacts, affiliations), general sample metadata (sample names, species, etc) and sample characteristics. All informat (URL_TO_INSERT_TERM_646 https://fairsharing.org/search?recordType=model_and_format) ion is stored in key: value pairs. In sample characteristics, only data with flatten structure is accepted. 

All cohort metadata and drug sensitivity data of each sample were combined into separate JSON (URL_TO_INSERT_RECORD-ABBREV_649 https://fairsharing.org/FAIRsharing.5bbab9)  files, named after the original sample identifier (URL_TO_INSERT_TERM_648 https://fairsharing.org/search?recordType=identifier_schema) .

BioSamples JSON (URL_TO_INSERT_RECORD-ABBREV_651 https://fairsharing.org/FAIRsharing.5bbab9)  schema only supports strings as value types. Therefore, complex data, for example, drug sensitivity data, need additional parsing to fit into the schema. For each drug response experiments, four descriptive attributes were “_Drug Name_”, “_Value_”, “_Unit_” and “_Drug response level_” provided. To convert them to key: value pairs, the “_drug name_” was kept as part of the attribute key. “Value”, “Unit” and “Drug response level” informat (URL_TO_INSERT_TERM_650 https://fairsharing.org/search?recordType=model_and_format) ion were joint into one string (URL_TO_INSERT_RECORD-NAME_652 https://fairsharing.org/FAIRsharing.9b7wvk) . For example, the drug response of _regorafenib in Sample 302_MET_CELL_XEN_ (Figure 6) was converted to _“Drug response(regorafenib): Minor response, IC50(µM) = 29”._ For drugs that were tested more than once, a replicate ID was added to the attribute key.



Administrative informat (URL_TO_INSERT_TERM_655 https://fairsharing.org/search?recordType=model_and_format) ion was also added to each JSON (URL_TO_INSERT_RECORD-ABBREV_656 https://fairsharing.org/FAIRsharing.5bbab9)  object, including institute, project (URL_TO_INSERT_TERM_653 https://fairsharing.org/search?recordType=project)  name, and project (URL_TO_INSERT_TERM_654 https://fairsharing.org/search?recordType=project)  website. Contact details were not provided, which needs further confirmation from the data owners.
...
Data curation was implemented to increase the findability of the extracted metadata, including ontology (URL_TO_INSERT_TERM_657 https://fairsharing.org/search?recordType=terminology_artefact)  mapping, encoding, and fixing data discrepancy.


#### Drug name ontology

Different types of drug names were used in the original drug sensitivity data, including generic names, trade names and their chemical names. Some in-development drugs were labelled by their company code. All drug names were mapped to a standard (URL_TO_INSERT_TERM_658 https://fairsharing.org/search?fairsharingRegistry=Standard) ized nomenclature, together with ontology (URL_TO_INSERT_TERM_659 https://fairsharing.org/search?recordType=terminology_artefact)  annotations, to increase the findability of drug response results.

The popular drug nomenclatures include International Nonproprietary Names (INNs), [DrugBank](https://www.drugbank.ca/) names, Chemical Entities of Biological Interest (URL_TO_INSERT_RECORD-NAME_666 https://fairsharing.org/FAIRsharing.62qk8w)  ([ChEBI](https://www.ebi.ac.uk/chebi/)) database (URL_TO_INSERT_TERM_660 https://fairsharing.org/search?fairsharingRegistry=Database)  identifier (URL_TO_INSERT_TERM_664 https://fairsharing.org/search?recordType=identifier_schema) s, and bioactivity database,[ChEMBL](https://www.ebi.ac.uk/chembl/) identifier (URL_TO_INSERT_TERM_665 https://fairsharing.org/search?recordType=identifier_schema) s. Among them, the ChEBI (URL_TO_INSERT_RECORD-ABBREV_667 https://fairsharing.org/FAIRsharing.62qk8w)  database (URL_TO_INSERT_TERM_661 https://fairsharing.org/search?fairsharingRegistry=Database)  covers most drugs in OncoTrack, is easy to access and has been curated as an ontology (URL_TO_INSERT_TERM_662 https://fairsharing.org/search?recordType=terminology_artefact)  source in Open Biomedical Ontology (URL_TO_INSERT_TERM_663 https://fairsharing.org/search?recordType=terminology_artefact)  (OBO (URL_TO_INSERT_RECORD-ABBREV_670 https://fairsharing.org/FAIRsharing.847069) ). Hence, the drug names in ChEBI (URL_TO_INSERT_RECORD-ABBREV_668 https://fairsharing.org/FAIRsharing.62qk8w)  were selected as the primary drug names. For drugs or active compounds that were not included in ChEBI (URL_TO_INSERT_RECORD-ABBREV_669 https://fairsharing.org/FAIRsharing.62qk8w) , their names in ChEMBL (URL_TO_INSERT_RECORD-NAME_671 https://fairsharing.org/FAIRsharing.m3jtpg)  were selected.

The OncoTrack drug names were mapped to corresponding ontology (URL_TO_INSERT_TERM_673 https://fairsharing.org/search?recordType=terminology_artefact)  terms in ChEBI (URL_TO_INSERT_RECORD-ABBREV_677 https://fairsharing.org/FAIRsharing.62qk8w)  and ChEMBL (URL_TO_INSERT_RECORD-NAME_680 https://fairsharing.org/FAIRsharing.m3jtpg)  using ZOOMA. ZOOMA is an ontology (URL_TO_INSERT_TERM_674 https://fairsharing.org/search?recordType=terminology_artefact)  annotation tool, which maps free texts to ontology (URL_TO_INSERT_TERM_675 https://fairsharing.org/search?recordType=terminology_artefact)  terms based on a curated repository (URL_TO_INSERT_TERM_672 https://fairsharing.org/search?recordType=repository)  of annotation knowledge. In OncoTrack drug name mapping, only ChEBI (URL_TO_INSERT_RECORD-ABBREV_678 https://fairsharing.org/FAIRsharing.62qk8w)  and ChEMBL (URL_TO_INSERT_RECORD-NAME_681 https://fairsharing.org/FAIRsharing.m3jtpg)  were selected as ontology (URL_TO_INSERT_TERM_676 https://fairsharing.org/search?recordType=terminology_artefact)  sources, and all curation sources except GWAS (URL_TO_INSERT_RECORD-ABBREV_683 https://fairsharing.org/FAIRsharing.ubYn8D)  were selected to improve the coverage and increase the mapping confidence. Figure 6 is an example of the ZOOMA mapping output. Only entries with high mapping confidences were selected. The other entries were mapped manually to terms in ChEBI (URL_TO_INSERT_RECORD-ABBREV_679 https://fairsharing.org/FAIRsharing.62qk8w) . Among all 17 drugs, Drug “BI 860585” didn’t map (URL_TO_INSERT_RECORD-NAME_682 https://fairsharing.org/FAIRsharing.53edcc)  to any pharmaceutical terms, so the original name was kept. The complete mapping results are listed [here](https://drive.google.com/open?id=1kYH76-3K3mkz6wfTydFoT_UPf8WP0fe_). 
...
The curated metadata, both in summary spreadsheet format (URL_TO_INSERT_TERM_691 https://fairsharing.org/search?recordType=model_and_format)  and JSON (URL_TO_INSERT_RECORD-ABBREV_693 https://fairsharing.org/FAIRsharing.5bbab9)  format (URL_TO_INSERT_TERM_692 https://fairsharing.org/search?recordType=model_and_format) , were uploaded to Owncloud and Google drive. The OncoTrack sample metadata can be submitted directly to the EMBL BioSamples database (URL_TO_INSERT_TERM_689 https://fairsharing.org/search?fairsharingRegistry=Database)  upon request. Supplementary figure 1 is an example of how the metadata will be displayed in the BioSamples database (URL_TO_INSERT_TERM_690 https://fairsharing.org/search?fairsharingRegistry=Database) . Checksums were also provided to verify metadata integrity. 


## Summary

The public cohort metadata and drug sensitivity data were converted to tab-delimited summary files. For each sample, corresponding cohort metadata and drug data were combined to a separate JSON (URL_TO_INSERT_RECORD-ABBREV_697 https://fairsharing.org/FAIRsharing.5bbab9)  file, which can be later loaded to EBI BioSamples database (URL_TO_INSERT_TERM_694 https://fairsharing.org/search?fairsharingRegistry=Database) . Metadata were translated to consistent terminologies (URL_TO_INSERT_TERM_695 https://fairsharing.org/search?recordType=terminology_artefact)  and linked with ontology (URL_TO_INSERT_TERM_696 https://fairsharing.org/search?recordType=terminology_artefact)  terms.


## Future plan

The current ontology (URL_TO_INSERT_TERM_699 https://fairsharing.org/search?recordType=terminology_artefact)  annotation is limited to drug names. Other ontologies (URL_TO_INSERT_TERM_700 https://fairsharing.org/search?recordType=terminology_artefact)  can be added upon request. More detailed administrative metadata will be added once getting permission from the OncoTrack consortium. The license will be added according to the drug release policy (URL_TO_INSERT_TERM_698 https://fairsharing.org/search?fairsharingRegistry=Policy) .
