(fcb-approach-cdis (URL_TO_INSERT_RECORD-NAME_13 https://fairsharing.org/3525) c)=
...
This recipe provides a general guide for mapping a clinical trial dataset to [CDISC Study Data Tabulated Model](https://doi.org/10.25504/FAIRsharing.s51qk5) (CDISC (URL_TO_INSERT_RECORD-NAME_15 https://fairsharing.org/3525) -SDTM) using practical examples from two Innovative Medicine Initiative project (URL_TO_INSERT_TERM_14 https://fairsharing.org/search?recordType=project) s, APPROACH and ABIRISK.

The recipe will cover the following elements:

> * Provide a general overview of the challenges of mapping a non-conforming data dictionary to CDISC (URL_TO_INSERT_RECORD-NAME_17 https://fairsharing.org/3525)  STDM standard (URL_TO_INSERT_TERM_16 https://fairsharing.org/search?fairsharingRegistry=Standard)  and CDISC (URL_TO_INSERT_RECORD-NAME_18 https://fairsharing.org/3525)  Vocabulary.
>
> * Illustrate the mapping process using the above project (URL_TO_INSERT_TERM_19 https://fairsharing.org/search?recordType=project) s.
>
> * Describe the Extract-Transform-Load (ETL) processes necessary to convert the data to CDISC (URL_TO_INSERT_RECORD-NAME_20 https://fairsharing.org/3525) -SDTM based on the mapped data dictionary.
...
---
name: approach-cdis (URL_TO_INSERT_RECORD-NAME_21 https://fairsharing.org/3525) c-figure0
height: 1000 px
alt: Recipe Steps
---
Recipe Steps
...
* Technical requirements:
   * Knowledge of relational database (URL_TO_INSERT_TERM_22 https://fairsharing.org/search?fairsharingRegistry=Database) s is an advantage
   * Fluency in a scripting language such as bash, Python or R
* Knowledge requirement:
   * A thorough understanding of CDISC (URL_TO_INSERT_RECORD-NAME_24 https://fairsharing.org/3525)  standard (URL_TO_INSERT_TERM_23 https://fairsharing.org/search?fairsharingRegistry=Standard) s, in particular CDISC (URL_TO_INSERT_RECORD-NAME_25 https://fairsharing.org/3525) -SDTM, is essential
...
| Data Format (URL_TO_INSERT_TERM_27 https://fairsharing.org/search?recordType=model_and_format) s  | Terminologies (URL_TO_INSERT_TERM_28 https://fairsharing.org/search?recordType=terminology_artefact)  | Model (URL_TO_INSERT_TERM_26 https://fairsharing.org/search?recordType=model_and_format) s  |
| :------------- | :------------- | :------------- |
| [CDISC-SDTM](https://www.cdisc.org/standards/foundational/sdtm)  |  [CDISC (URL_TO_INSERT_RECORD-NAME_29 https://fairsharing.org/3525)  terminology](https://www.cdisc.org/standards/terminology/controlled-terminology) | |
...
[CDISC-SDTM](https://www.cdisc.org/standards/foundational/sdtm) is a standard (URL_TO_INSERT_TERM_30 https://fairsharing.org/search?fairsharingRegistry=Standard)  model (URL_TO_INSERT_TERM_31 https://fairsharing.org/search?recordType=model_and_format)  and framework for organising, 
annotating and format (URL_TO_INSERT_TERM_32 https://fairsharing.org/search?recordType=model_and_format) ting data from clinical trials {footcite}`sdtm`. Regulatory agencies, such as the
[US FDA](https://www.fda.gov/media/122970/download), require clinical trial results to be submitted in this format (URL_TO_INSERT_TERM_33 https://fairsharing.org/search?recordType=model_and_format) . 
...
Before any mapping can take place, it is essential to gain a good understanding of the project (URL_TO_INSERT_TERM_34 https://fairsharing.org/search?recordType=project) 's data dictionary, i.e. 
its list of variables, their types, definitions and domains. 

A good data dictionary will already split properties into domains, for example: by data collection (URL_TO_INSERT_TERM_35 https://fairsharing.org/search?recordType=collection)  instrument or other data source. 
...
* The ABIRISK data dictionary already used a similar setup to CDISC (URL_TO_INSERT_RECORD-NAME_36 https://fairsharing.org/3525) -SDTM, which made this step relatively straight-forward. 
...
* Derivation - some values might be derived from another value, for example using a logical decision tree "if A=x, then value=y" or transformed, e.g. converting a laboratory result reported in non-standard (URL_TO_INSERT_TERM_38 https://fairsharing.org/search?fairsharingRegistry=Standard)  units into standard (URL_TO_INSERT_TERM_39 https://fairsharing.org/search?fairsharingRegistry=Standard)  units.
...
One major issue that we encountered with both our mapping project (URL_TO_INSERT_TERM_46 https://fairsharing.org/search?recordType=project) s was trial/study data coverage gaps from the respective
study data dictionaries. 
The SDTM includes an entire area called the "Trial Design Model (URL_TO_INSERT_TERM_47 https://fairsharing.org/search?recordType=model_and_format) ", which provides a structured way of representing all 
activities of a clinical trial, such as planned visits, treatment arms and inclusion and exclusion criteria. 
While this informat (URL_TO_INSERT_TERM_48 https://fairsharing.org/search?recordType=model_and_format) ion is a key component of a completely SDTM-compliant dataset, it was not part of the data dictionary
...
In order to build those links, actual subject, specimen and visit informat (URL_TO_INSERT_TERM_49 https://fairsharing.org/search?recordType=model_and_format) ion is needed. This informat (URL_TO_INSERT_TERM_50 https://fairsharing.org/search?recordType=model_and_format) ion is specified 
...
* We presented an overview of our approach to mapping project (URL_TO_INSERT_TERM_51 https://fairsharing.org/search?recordType=project) -specific data dictionaries to the CDISC (URL_TO_INSERT_RECORD-NAME_52 https://fairsharing.org/3525) -SDTMIG with a view to transforming the corresponding datasets to be SDTM-compliant.
* The CDISC (URL_TO_INSERT_RECORD-NAME_56 https://fairsharing.org/3525) -SDTM standard (URL_TO_INSERT_TERM_53 https://fairsharing.org/search?fairsharingRegistry=Standard)  supports the interoperability between datasets due to its high level of standard (URL_TO_INSERT_TERM_54 https://fairsharing.org/search?fairsharingRegistry=Standard) isation, detailed model (URL_TO_INSERT_TERM_55 https://fairsharing.org/search?recordType=model_and_format) ling and widespread use.
* The high level of detailed knowledge of the standard (URL_TO_INSERT_TERM_57 https://fairsharing.org/search?fairsharingRegistry=Standard)  required to successfully convert a dataset to SDTM presents a significant hurdle 
