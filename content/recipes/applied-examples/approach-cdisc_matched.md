...
| :------------- | :------------- | :------------- |
| [CDISC-SDTM](https://www.cdisc.org (URL_TO_INSERT_RECORD_45 https://fairsharing.org/3525) /standards/foundational/sdtm (URL_TO_INSERT_RECORD_43 https://fairsharing.org/FAIRsharing.s51qk5) )  |  [CDISC (URL_TO_INSERT_RECORD_44 https://fairsharing.org/3525)  terminology](https://www.cdisc.org (URL_TO_INSERT_RECORD_46 https://fairsharing.org/3525) /standards/terminology (URL_TO_INSERT_RECORD_42 https://fairsharing.org/FAIRsharing.szrmev) /controlled-terminology) | |
| [CDISC-SDTM v2](https://www.cdisc.org (URL_TO_INSERT_RECORD_48 https://fairsharing.org/3525) /standards/foundational/sdtm (URL_TO_INSERT_RECORD_47 https://fairsharing.org/FAIRsharing.s51qk5) /sdtm-v2-0)| | |
| [CDISC-SDTMIG 3.4](https://www.cdisc.org (URL_TO_INSERT_RECORD_50 https://fairsharing.org/3525) /standards/foundational/sdtm (URL_TO_INSERT_RECORD_49 https://fairsharing.org/FAIRsharing.s51qk5) ig/sdtmig-v3-4)| | |
---

## General challenges of mapping to CDISC-SDTM

[CDISC-SDTM](https://www.cdisc.org (URL_TO_INSERT_RECORD_54 https://fairsharing.org/3525) /standards/foundational/sdtm (URL_TO_INSERT_RECORD_53 https://fairsharing.org/FAIRsharing.s51qk5) ) is a standard (URL_TO_INSERT_TERM_51 https://fairsharing.org/search?fairsharingRegistry=Standard)  model (URL_TO_INSERT_TERM_52 https://fairsharing.org/search?recordType=model_and_format)  and framework for organising, 
annotating and format (URL_TO_INSERT_TERM_55 https://fairsharing.org/search?recordType=model_and_format) ting data from clinical trials {footcite}`sdtm`. Regulator (URL_TO_INSERT_RECORD_56 https://fairsharing.org/FAIRsharing.ey49c6) y agencies, such as the
[US FDA](https://www.fda.gov/media/122970/download), require clinical trial results to be submitted in this format (URL_TO_INSERT_TERM_57 https://fairsharing.org/search?recordType=model_and_format) . 
...
The SDTM Implementation Guide ([SDTMIG](https://www.cdisc.org (URL_TO_INSERT_RECORD_59 https://fairsharing.org/3525) /standards/foundational/sdtm (URL_TO_INSERT_RECORD_58 https://fairsharing.org/FAIRsharing.s51qk5) ig)) provides expanded guidance
...
[on the CDISC website](https://www.cdisc.org (URL_TO_INSERT_RECORD_61 https://fairsharing.org/3525) /standards/foundational/sdtm (URL_TO_INSERT_RECORD_60 https://fairsharing.org/FAIRsharing.s51qk5) ig/primer) {footcite}`sdtmig_v3_4`.
...
Before any map (URL_TO_INSERT_RECORD_63 https://fairsharing.org/FAIRsharing.53edcc) ping can take place, it is essential to gain a good understanding of the project (URL_TO_INSERT_TERM_62 https://fairsharing.org/search?recordType=project) 's data dictionary, i.e. 
its list of variables, their types, definitions and domains. 

A good data dictionary will already split properties into domains, for example: by data collection (URL_TO_INSERT_TERM_64 https://fairsharing.org/search?recordType=collection)  instrument or other data source. 
...
* The ABIRIS (URL_TO_INSERT_RECORD_65 https://fairsharing.org/FAIRsharing.e20b79) K data dictionary already used a similar setup to CDI (URL_TO_INSERT_RECORD_66 https://fairsharing.org/FAIRsharing.yzagph) SC (URL_TO_INSERT_RECORD_67 https://fairsharing.org/3525) -SDTM, which made this step relatively straight-forward. 
...
initial list had to be revised several times during the variable map (URL_TO_INSERT_RECORD_68 https://fairsharing.org/FAIRsharing.53edcc) ping process, when variables were identified that 
...
* Derivation - some values might be derived from another value, for example using a logical decision tree "if A=x, then value=y" or transformed, e.g. converting a laboratory result reported in non-standard (URL_TO_INSERT_TERM_71 https://fairsharing.org/search?fairsharingRegistry=Standard)  units into standard (URL_TO_INSERT_TERM_72 https://fairsharing.org/search?fairsharingRegistry=Standard)  units.
...
One major issue that we encountered with both our map (URL_TO_INSERT_RECORD_82 https://fairsharing.org/FAIRsharing.53edcc) ping project (URL_TO_INSERT_TERM_81 https://fairsharing.org/search?recordType=project) s was trial/study data coverage gaps from the respective
study data dictionaries. 
The SDTM includes an entire area called the "Trial Design Model (URL_TO_INSERT_TERM_83 https://fairsharing.org/search?recordType=model_and_format) ", which provides a structured way of representing all 
activities of a clinical trial, such as planned visits, treatment arms and inclusion and exclusion criteria. 
While this informat (URL_TO_INSERT_TERM_84 https://fairsharing.org/search?recordType=model_and_format) ion is a key component of a completely SDTM-compliant dataset, it was not part of the data dictionary
and was therefore not part of the map (URL_TO_INSERT_RECORD_85 https://fairsharing.org/FAIRsharing.53edcc) ping strategy. 
As a result, any ETL process covering the data alone would not have generated a fully SDTM-compliant dataset.
Another area of the SDTMIG we did not include in our map (URL_TO_INSERT_RECORD_86 https://fairsharing.org/FAIRsharing.53edcc) pings was the representation of relationships in the data,
...
In order to build those links, actual subject, specimen and visit informat (URL_TO_INSERT_TERM_87 https://fairsharing.org/search?recordType=model_and_format) ion is needed. This informat (URL_TO_INSERT_TERM_88 https://fairsharing.org/search?recordType=model_and_format) ion is specified 
...
* We presented an overview of our approach to map (URL_TO_INSERT_RECORD_91 https://fairsharing.org/FAIRsharing.53edcc) ping project (URL_TO_INSERT_TERM_89 https://fairsharing.org/search?recordType=project) -specific data dictionaries to the CDI (URL_TO_INSERT_RECORD_90 https://fairsharing.org/FAIRsharing.yzagph) SC (URL_TO_INSERT_RECORD_92 https://fairsharing.org/3525) -SDTMIG with a view to transforming the corresponding datasets to be SDTM-compliant.
* The CDI (URL_TO_INSERT_RECORD_96 https://fairsharing.org/FAIRsharing.yzagph) SC (URL_TO_INSERT_RECORD_97 https://fairsharing.org/3525) -SDTM standard (URL_TO_INSERT_TERM_93 https://fairsharing.org/search?fairsharingRegistry=Standard)  supports the interoperability between datasets due to its high level of standard (URL_TO_INSERT_TERM_94 https://fairsharing.org/search?fairsharingRegistry=Standard) isation, detailed model (URL_TO_INSERT_TERM_95 https://fairsharing.org/search?recordType=model_and_format) ling and widespread use.
* The high level of detailed knowledge of the standard (URL_TO_INSERT_TERM_98 https://fairsharing.org/search?fairsharingRegistry=Standard)  required to successfully convert a dataset to SDTM presents a significant hurdl (URL_TO_INSERT_RECORD_99 https://fairsharing.org/FAIRsharing.vm4688) e 
