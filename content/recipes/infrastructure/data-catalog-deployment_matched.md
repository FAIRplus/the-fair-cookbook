...
This recipe is a step-by-step guide on how to deploy the IMI Data Catalog (URL_TO_INSERT_RECORD_2190 https://fairsharing.org/FAIRsharing.de533c) ue in Docker. 

## Introduction

For a more general introduction to data catalogues, their elements and data model (URL_TO_INSERT_TERM_2191 https://fairsharing.org/search?recordType=model_and_format) s, 
see the [data catalogue recipe](https://www.TODO.uldatacatalog.ul). 
This recipe is intended as a set of step-by-step instructions to deploy via Docker the IMI Data Catalog (URL_TO_INSERT_RECORD_2192 https://fairsharing.org/FAIRsharing.de533c) ue
developed (URL_TO_INSERT_RECORD_2193 https://fairsharing.org/FAIRsharing.31385c)  at the Luxembourg Centre for Systems Biomedicine. The overall purpose of the data catalogue is to host 
dataset-level metadata for a wide range of IMI project (URL_TO_INSERT_TERM_2194 https://fairsharing.org/search?recordType=project) s. Datasets are FAIR (URL_TO_INSERT_RECORD_2195 https://fairsharing.org/FAIRsharing.WWI10U) ified and search (URL_TO_INSERT_RECORD_2196 https://fairsharing.org/FAIRsharing.52b22c) able by a range facets. 
The catalogue is not intended to hold the actual data, although it provides links to where the data is hosted, 
together with informat (URL_TO_INSERT_TERM_2197 https://fairsharing.org/search?recordType=model_and_format) ion on any access restrictions.
...
* First, generate the certificates that will be used to enable HTTPS (URL_TO_INSERT_RECORD_2198 https://fairsharing.org/FAIRsharing.cd2f9e)  in reverse proxy. To do so, execute:
...
```{warning}
⚡ However, be aware that skipping this would cause the HTTPS (URL_TO_INSERT_RECORD_2199 https://fairsharing.org/FAIRsharing.cd2f9e)  connection to be unsafe!
```
...
* Edit the **settings.py** file to add a random string (URL_TO_INSERT_RECORD_2200 https://fairsharing.org/FAIRsharing.9b7wvk)  of characters in **SECRET_KEY** attribute. For maximum security,
...
* In a new terminal, to create `Solr` core (URL_TO_INSERT_RECORD_2201 https://fairsharing.org/FAIRsharing.m283c)  (URL_TO_INSERT_RECORD_2202 https://fairsharing.org/FAIRsharing.xMmOCL) s, do:
...
* Then, still in the second terminal, put Solr data into the core (URL_TO_INSERT_RECORD_2203 https://fairsharing.org/FAIRsharing.m283c)  (URL_TO_INSERT_RECORD_2204 https://fairsharing.org/FAIRsharing.xMmOCL) s:  
...
the project (URL_TO_INSERT_TERM_2205 https://fairsharing.org/search?recordType=project) , the container has to be rebuilt in order to see changes in application** :
...
This will remove any persisted data - you must redo **solr create_core (URL_TO_INSERT_RECORD_2206 https://fairsharing.org/FAIRsharing.m283c)  (URL_TO_INSERT_RECORD_2207 https://fairsharing.org/FAIRsharing.xMmOCL) ** (see step 4 in the previous section) to 
recreate the Solr core (URL_TO_INSERT_RECORD_2208 https://fairsharing.org/FAIRsharing.m283c)  (URL_TO_INSERT_RECORD_2209 https://fairsharing.org/FAIRsharing.xMmOCL) s.
...
This recipe provides a step-by-step guide to deploying the **IMI data catalogue** developed (URL_TO_INSERT_RECORD_2210 https://fairsharing.org/FAIRsharing.31385c)  at [University of Luxembourg](https://wwwen.uni.lu/lcsb),
