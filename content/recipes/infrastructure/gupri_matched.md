...
In the ideal linked data world, best practice is to provide a URL(URL_TO_INSERT_RECORD https://tools.ietf.org/html/rfc1630) which can be accessed via HTTP, more specifically the HTTP GET command.
...
to contact and ask for content regarding the URL(URL_TO_INSERT_RECORD https://tools.ietf.org/html/rfc1630) you want to resolve.

DNS uses the concept of hierarch(URL_TO_INSERT_RECORD https://arch.library.northwestern.edu/)ical domains to resolve URL(URL_TO_INSERT_RECORD https://tools.ietf.org/html/rfc1630)s. 
> (Learn more about subdomains here: [https://en.wikipedia.org/wiki/Subdomain](https://en.wikipedia.org/wiki/Subdomain)) 

For example, you might want to resolve the following URL(URL_TO_INSERT_RECORD https://tools.ietf.org/html/rfc1630):
...
and ask there who is responsible for managing the domain "wikiped(URL_TO_INSERT_RECORD https://www.cog-genomics.org/plink2/formats#ped)ia.org",
and finally who is responsible for managing "en.wikiped(URL_TO_INSERT_RECORD https://www.cog-genomics.org/plink2/formats#ped)ia.org". 
...
In an enterprise context, this might be used to delegate authority within hierarch(URL_TO_INSERT_RECORD https://arch.library.northwestern.edu/)y:

http://my-department.my-enterprise.com/my-resource

can be understood  in the following sense: "my-enterprise" allowed "my-department" to "mint", i.e. generate / provide / resolve, URL(URL_TO_INSERT_RECORD https://tools.ietf.org/html/rfc1630)s over the internet. 

If even more granularity / hierarch(URL_TO_INSERT_RECORD https://arch.library.northwestern.edu/)y is desired, an URL(URL_TO_INSERT_RECORD https://tools.ietf.org/html/rfc1630) may also look like this:
...
In the following recipe you will learn about w3id(URL_TO_INSERT_RECORD https://w3id.org/).org(URL_TO_INSERT_RECORD https://w3id.org/), purl.org, and the DOI(URL_TO_INSERT_RECORD https://www.doi.org) / handle(URL_TO_INSERT_RECORD http://handle.net) system, 
...
To access the graphical user interface / web frontend, you need to register with arch(URL_TO_INSERT_RECORD https://arch.library.northwestern.edu/)ive.org. 
...
1.	Register on `arch(URL_TO_INSERT_RECORD https://arch.library.northwestern.edu/)ive.org`
2.	Log-in to `purl.org`
3.	Create your domain, e.g. `purl.org/faircookbook(URL_TO_INSERT_RECORD http://bioportal.bioontology.org/ontologies/3059)`
...
The service `w3id(URL_TO_INSERT_RECORD https://w3id.org/).org(URL_TO_INSERT_RECORD https://w3id.org/)` emerged when `purl.org` was down {footcite}`w3id(URL_TO_INSERT_RECORD https://w3id.org/)`. 
It is run by multiple stakeholders (see self-description on w3id(URL_TO_INSERT_RECORD https://w3id.org/).org(URL_TO_INSERT_RECORD https://w3id.org/)) bound by a "social contract". 

Management is more difficult than purl.org, because w3id(URL_TO_INSERT_RECORD https://w3id.org/).org(URL_TO_INSERT_RECORD https://w3id.org/) does not have a graphical user interface / frontend, 
but relies on "raw" `.htaccess` files deposited on GitHub(URL_TO_INSERT_RECORD https://github.com/)(URL_TO_INSERT_RECORD https://github.com/).com. 
...
After making a proposal to change the redirection rules via a GitHub(URL_TO_INSERT_RECORD https://github.com/)(URL_TO_INSERT_RECORD https://github.com/) Pull Request, maintainers of the w3id(URL_TO_INSERT_RECORD https://w3id.org/).org(URL_TO_INSERT_RECORD https://w3id.org/) team
...
whereas The Handle(URL_TO_INSERT_RECORD http://handle.net)(URL_TO_INSERT_RECORD http://handle.net) System is quite inexpensive (approx. 50 €/$ per year). 

There is also a way to get handle(URL_TO_INSERT_RECORD http://handle.net)s for free, which is described here:

You can reach out to `http://grnet.gr` – e.g. through the subpage `https://epic.grnet.gr/` where there is a Contact form. 

An alternative is http://www.pidconsortium.net(URL_TO_INSERT_RECORD http://www.pidconsortium.net/)/ (of which grnet is a member).
...
2.	Make yourself fam(URL_TO_INSERT_RECORD https://www.cog-genomics.org/plink2/formats#fam)iliar with the API (no easy-to-use graphical user interface available as of now) – see http://www.handle.net(URL_TO_INSERT_RECORD http://handle.net)/tech_manual/HN_Tech_Manual_9.pdf 
3.	Make the API call to register a handle(URL_TO_INSERT_RECORD http://handle.net)
