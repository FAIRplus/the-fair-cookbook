...
This recipe is a step-by-step guide on how to deploy the EBI Ontology Lookup Service (URL_TO_INSERT_RECORD-NAME_1182 https://fairsharing.org/FAIRsharing.Mkl9RR)  (OLS (URL_TO_INSERT_RECORD-ABBREV_1183 https://fairsharing.org/FAIRsharing.Mkl9RR) ) {footcite}`pmid20460452`, {footcite}`ols_jupp` on local machines. 
...
Ontology Lookup Service (URL_TO_INSERT_RECORD-NAME_1184 https://fairsharing.org/FAIRsharing.Mkl9RR)  is an open-source ontology management service developed by [EMBL-EBI](https://www.ebi.ac.uk/).

It is a repository for biomedical ontologies, and serves as a single point of access to query, browse and navigate
different ontologies. 

OLS (URL_TO_INSERT_RECORD-ABBREV_1185 https://fairsharing.org/FAIRsharing.Mkl9RR)  supports the [Open Biological and Biomedical Ontology (OBO) Foundry](http://www.obofoundry.org/)
guidelines and connects with other ontology services. 

It provides both web interface and API to search and browser 
ontologies. 
[Recipe FCB003](https://w3id.org/faircookbook/FCB003) provides a detailed description of OLS (URL_TO_INSERT_RECORD-ABBREV_1186 https://fairsharing.org/FAIRsharing.Mkl9RR) .

A local OLS (URL_TO_INSERT_RECORD-ABBREV_1187 https://fairsharing.org/FAIRsharing.Mkl9RR)  allows users to protect and control their ontology-related data, and make stable and fast access to ontology
services possible. 
...
[OWL](https://fairsharing.org/10.25504/FAIRsharing.atygwy) formats can be loaded to OLS (URL_TO_INSERT_RECORD-ABBREV_1188 https://fairsharing.org/FAIRsharing.Mkl9RR)  by adding ontology
...
Below is an example configuration of the Experimental Factor Ontology (URL_TO_INSERT_RECORD-NAME_1189 https://fairsharing.org/FAIRsharing.1gr4tz)  (EFO (URL_TO_INSERT_RECORD-ABBREV_1190 https://fairsharing.org/FAIRsharing.1gr4tz) ) provided
...
To add local ontologies, the ontology files need to be first copied to the **OLS (URL_TO_INSERT_RECORD-ABBREV_1191 https://fairsharing.org/FAIRsharing.Mkl9RR) -docker** directory. 
...
Most reference ontologies use the OBO foundry (URL_TO_INSERT_RECORD-NAME_1192 https://fairsharing.org/FAIRsharing.847069)  Permanent URLs (PURLs). 

The PURLs can be found [here](http://www.obofoundry.org/). 

For example, the location of Data Usage Ontology (DUO (URL_TO_INSERT_RECORD-ABBREV_1193 https://fairsharing.org/FAIRsharing.5dnjs2) ) can be specified by adding:
...
[EBI OLS](https://www.ebi.ac.uk/ols/index) or the [OBO Foundry (URL_TO_INSERT_RECORD-NAME_1194 https://fairsharing.org/FAIRsharing.847069) ](https://obofoundry.org/) {footcite}`pmid17989687obofoundry2007`,
{footcite}`pmid34697637obofoundry2021` .

#### 2.1 Get ontology metadata from the EBI OLS

For ontologies included in the EBI OLS (URL_TO_INSERT_RECORD-ABBREV_1195 https://fairsharing.org/FAIRsharing.Mkl9RR) , the metadata can be downloaded directly using the EBI OLS (URL_TO_INSERT_RECORD-ABBREV_1196 https://fairsharing.org/FAIRsharing.Mkl9RR)  endpoint,
...
[Here](https://www.ebi.ac.uk/ols/ontologies) is a list of all the ontologies available at OLS (URL_TO_INSERT_RECORD-ABBREV_1197 https://fairsharing.org/FAIRsharing.Mkl9RR) , along with their
respective "short name" and other information. 

For example, the following command downloads the ontology metadata of EFO (URL_TO_INSERT_RECORD-ABBREV_1198 https://fairsharing.org/FAIRsharing.1gr4tz)  and Adverse Event Reporting Ontology (AERO) 
...
```
To avoid losing pre-loaded ontologies, the metadata of EFO (URL_TO_INSERT_RECORD-ABBREV_1199 https://fairsharing.org/FAIRsharing.1gr4tz)  and AERO can also be appended to the already existing **ols-config.yml** using:
...
For ontologies that are in the OBO foundry (URL_TO_INSERT_RECORD-NAME_1200 https://fairsharing.org/FAIRsharing.847069) , the metadata can also be downloaded from the [OBO Foundry GitHub repository](https://github.com/OBOFoundry/purl.obolibrary.org/tree/master/config).

Additional formatting is required for metadata downloaded from the OBO foundry (URL_TO_INSERT_RECORD-NAME_1201 https://fairsharing.org/FAIRsharing.847069) .
...
The local OLS (URL_TO_INSERT_RECORD-ABBREV_1202 https://fairsharing.org/FAIRsharing.Mkl9RR)  service can be accessed at http://localhost:8080/index 

### 4. Manage ontologies
OLS (URL_TO_INSERT_RECORD-ABBREV_1203 https://fairsharing.org/FAIRsharing.Mkl9RR)  allows the addition, update, and removal of ontologies.
...
The OLS (URL_TO_INSERT_RECORD-ABBREV_1204 https://fairsharing.org/FAIRsharing.Mkl9RR)  container can be stopped and removed by providing the container name. 

According to the parameters presented on the previous Docker creation command block, 
the name of the OLS (URL_TO_INSERT_RECORD-ABBREV_1205 https://fairsharing.org/FAIRsharing.Mkl9RR)  Docker container is "OLS (URL_TO_INSERT_RECORD-ABBREV_1206 https://fairsharing.org/FAIRsharing.Mkl9RR) ": 
...
If more than one ontology are going to be loaded into OLS (URL_TO_INSERT_RECORD-ABBREV_1207 https://fairsharing.org/FAIRsharing.Mkl9RR)  from disk, the **Dockerfile** needs modifying before 
...
## Conclusion
The local OLS (URL_TO_INSERT_RECORD-ABBREV_1208 https://fairsharing.org/FAIRsharing.Mkl9RR)  provides API endpoints for retrieving, submitting, updating, and querying ontology data, as well as a 
...
A detailed description of OLS (URL_TO_INSERT_RECORD-ABBREV_1209 https://fairsharing.org/FAIRsharing.Mkl9RR)  functions can be found in the built-in documentation page.

To customize the local OLS (URL_TO_INSERT_RECORD-ABBREV_1210 https://fairsharing.org/FAIRsharing.Mkl9RR)  user interface, for example, adding corporate logos, please check the OLS (URL_TO_INSERT_RECORD-ABBREV_1211 https://fairsharing.org/FAIRsharing.Mkl9RR)  source code
