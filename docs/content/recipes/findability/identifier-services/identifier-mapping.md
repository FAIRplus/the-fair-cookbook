...
The need for each database to mint their own identifier is a result of each taking a different perspective on the concept. The database owners should not pre-determine for the users of the data what identifiers in another database are equivalent, but they should provide indications 
